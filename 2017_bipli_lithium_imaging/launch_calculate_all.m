function launch_calculate_all(subjectdir)    
    ...
    splitfolder=splitfolder(1:end-1);
    ...
    LifilesS=dir(strcat(subjectdir,'\TPI\Reconstruct_gridding\03-Concentration\*.nii'));
    i=1;
    Lifiles=cell(size(LifilesS,1),1);
    for Lifile=LifilesS'
        Lifiles{i}=strcat(subjectdir,'\TPI\Reconstruct_gridding\03-Concentration\',Lifile.name);
        i=i+1;
    end
