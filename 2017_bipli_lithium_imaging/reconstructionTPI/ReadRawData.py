	...
	with open(source_file, 'rb') as source:
		...
		for line in source: 
			...
			for a in rx8.findall(str(line)):	
				...
				res=([str(s) for s in line2.split()])
				if len(res)>1:
					oversamplingFactor=res[4]
					oversamplingFactor=oversamplingFactor[0]
			for a in rx9.findall(str(line)):
			...
			for a in rx11.findall(str(line)):
				line2=line.decode("utf-8")
				PhaseFOV=([str(s) for s in line2.split()])               
				PhaseFOV=PhaseFOV[2]
				PhaseFOV=float(PhaseFOV)
			for a in rx12.findall(str(line)):
				ReadoutFOV=([int(s) for s in line.split() if s.isdigit()])		
			...
			for a in rx23.findall(str(line)):
				line2=line.decode("utf-8")
				TPI_linear_proportion=([str(s) for s in line2.split()])
...
def Read_NS_ME_TPIBlock(source_file, position, nbPoints, oversamplingFactor,coils,echoes):
	#with open(source_file, 'rb') as source:
	#with open(source_file, 'rt', encoding="utf-8") as source:       
	with open(source_file,'rb') as source:
		cplx=[]
...
#		position=position+28352
		...
		for e in range(echoes):
			for i in range(coils):	
				# print e,i
				hdr=Kline[e*(((coils)*(DataLinediv))+((coils)*128))+i*((DataLinediv)+128):e*(((coils)*(DataLinediv))+((coils)*128))+(i*(DataLinediv))+((i+1)*128)]
				signal=Kline[e*(((coils)*(DataLinediv))+((coils)*128))+(i*(DataLinediv))+((i+1)*128):e*(((coils)*(DataLinediv))+((coils)*128))+((i+1)*(DataLinediv))+((i+1)*128)]
				# print struct.unpack('f',signal[0:4])
