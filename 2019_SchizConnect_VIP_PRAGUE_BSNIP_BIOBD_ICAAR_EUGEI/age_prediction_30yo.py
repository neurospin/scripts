...
# -*- coding: utf-8 -*-
"""
Created on Sat Jul 27 12:20:07 2019
...
assert Residuals_ctrl_30_bySite.shape == (313, 142)
...
print("Test r2:%.2f" % metrics.r2_score(y, y_test_pred)) # Test r2:-0.04
...
assert Residuals_ctrl_BIOBD_30.shape == (101, 142)
...
assert Residuals_ies.shape == (80, 142) 
...
print("Test r2:%.2f" % metrics.r2_score(age_ctrl_30, age_ctrl_30_predicted)) # Test r2:0.28
...
print("Test r2:%.2f" % metrics.r2_score(y_ies_age, y_ies_age_predicted)) # Test r2:-0.35
...
model.fit(Residuals_ctrl_30_bySite_s, age_ctrl_30)
age_ctrl_30_predicted = model.predict(Residuals_ctrl_30_bySite_s)
print("Test r2:%.2f" % metrics.r2_score(age_ctrl_30, age_ctrl_30_predicted)) # Test r2:0.32
...
Residuals_ies_s = scaler.transform(Residuals_ies)
y_ies_age_predicted = model.predict(Residuals_ies_s)
print("Test r2:%.2f" % metrics.r2_score(y_ies_age, y_ies_age_predicted)) # Test r2:-0.33
...
print("Test r2:%.2f" % metrics.r2_score(y, y_test_pred)) # Test r2:0.18
print(model.best_params_) # {'alpha': 1000.0}
...
assert Residuals_ctrl_30.shape == (313, 142)
...
Residuals_ies_s = scaler.transform(Residuals_ies)
y_ies_age_predicted = model.predict(Residuals_ies_s)
print("Test r2:%.2f" % metrics.r2_score(y_ies_age, y_ies_age_predicted)) # Test r2:-0.48
...
Residuals_ctrl_BIOBD_30_s = scaler.transform(Residuals_ctrl_BIOBD_30)
y_ctrl_BIOBD_30_age_predicted = model.predict(Residuals_ctrl_BIOBD_30_s)
print("Test r2:%.2f" % metrics.r2_score(age_ctrl_BIOBD_30, y_ctrl_BIOBD_30_age_predicted)) # Test r2:0.15
...
print("Test r2:%.2f" % metrics.r2_score(y, y_test_pred)) # Test r2:0.38
print(model.best_params_) # {'alpha': 100.0}
...
# Y A-T-IL UNE CORRELATION ENTRE VIEILLISSEMENT CEREBRAL ET VIEILLISSEMENT METHYLOMIQUE ?
...
subjects = pd.read_csv('/home/anton/Dropbox/PhD/Article_Age_epigenetique_et_cerebral/2019_Brain_age/Analysis_all_data/data/conversion_UHR_data.tsv', sep='\t')
subjects = subjects['participant_id']

brain_age = pd.DataFrame(y_ies_age_predicted)
real_age = pd.DataFrame(y_ies_age)
status = pd.DataFrame(y_ies_status)
data_final = pd.concat([subjects, real_age, brain_age, status], axis=1)
data_final.columns = ['participant_id','real age', 'predicted age', 'clinical status']
data_final['age_diff'] = data_final['predicted age'] - data_final['real age']
assert data_final['real age'].mean() == 21.5
assert data_final['real age'].median() == 21.0
...
path='/home/anton/Dropbox/PhD/Article_Age_epigenetique_et_cerebral/2019_Methylation_age'

data = pd.read_csv(os.path.join(path,'methylation_DNA_AGE_ICAAR_final_exclusionIC05001.csv'))
data = data[['TrueID','Transition','Age_M0','Age_MF','Sexe','DNAmAge_M0','DNAmAge_MF']]
data['delta_time'] = data['Age_MF'] - data['Age_M0']
data['delta_DNAmTime'] = data['DNAmAge_MF'] - data['DNAmAge_M0']
data.Transition = data.Transition.apply(str)
data.Transition = data.Transition.map({'1':'yes','0':'no'})        
data = data.dropna()
data.head()
data['Age_acceleration'] = data[(data.Transition == 'yes') | (data.Transition == 'no')].delta_DNAmTime / data[(data.Transition == 'yes') | (data.Transition == 'no')].delta_time
assert data.shape == (38, 10)
data = data.rename(columns = {'TrueID':'code_icaar_start'})


pheno = pd.read_csv('/home/anton/Dropbox/PhD/Article_Age_epigenetique_et_cerebral/2019_Brain_age/Analysis_all_data/phenotype/clinic_icaar_201907.tsv', sep='\t')
pheno = pheno[['participant_id', 'code_icaar_start']]

data = pd.merge(pheno, data, on='code_icaar_start')
assert data.shape == (11, 11)


brainmethyl = pd.merge(data, data_final, on='participant_id')
brainmethyl[['code_icaar_start','Transition','clinical status','Age_acceleration','age_diff']]
#IC01006         no                1
#IC01101         no                1

brainmethyl.Transition.iloc[1] = 'yes'
brainmethyl.Transition.iloc[5] = 'yes'

data = brainmethyl[['code_icaar_start','Transition','clinical status','Age_acceleration','age_diff']]

# Test correlation between brain and epigenetic ageing.

from scipy.stats import spearmanr
spearmanr(data.Age_acceleration, data.age_diff, axis=0)
# SpearmanrResult(correlation=0.06363636363636364, pvalue=0.8525390725425217

fig, ax = plt.subplots()
fig.set_size_inches(18.5,10.5)
sns.scatterplot(x='Age_acceleration', y='age_diff', hue='Transition', hue_order=['no','yes'], data=data, ax=ax)
fig.suptitle("Epigenetic and brain age accelerations \n in converters and non-converters", fontsize=18)
plt.ylabel('Brain ageing', fontsize=16)
plt.xlabel('Epigenetic ageing', fontsize=16)

lm = sns.lmplot(x='Age_acceleration', y='age_diff', data=data, fit_reg=True)
fig = lm.fig
fig.set_size_inches(18.5,10.5)
fig.suptitle("Variance explained (R²) = 38%", fontsize=18)
plt.ylabel('Predicted (Brain) Age', fontsize=16)
...
#    return x[abs(x - x.median()) < 3 * mad]
#
#mad_Age_acceleration = 1.4826 * np.median(np.abs(data.Age_acceleration - data.Age_acceleration.median()))
#Age_acceleration_no_out = reject_outliers(data.Age_acceleration, mad_Age_acceleration)
#
#assert len(Age_acceleration_no_out) == 36 
