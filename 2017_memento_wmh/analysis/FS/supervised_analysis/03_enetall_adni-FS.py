...
WD = "/neurospin/brainomics/2017_memento/analysis/FS/results/supervised_analysis/enetall_adni"
WD_CLUSTER = WD.replace("/neurospin/", "/mnt/neurospin/sel-poivre/")
DATA_PATH = '/neurospin/brainomics/2017_memento/analysis/FS/data/adni'
user_func_filename = "/home/ad247405/git/scripts/2017_memento_wmh/analysis/FS/supervised_analysis/03_enetall_adni-FS.py"
...
def init():
    ...
    shutil.copy(os.path.join(DATA_PATH, 'X_mci_only.npy'), WD)
    shutil.copy(os.path.join(DATA_PATH, 'y_mci_only.npy'), WD)
    ...
    y= np.load(os.path.join(DATA_PATH, 'y_mci_only.npy'))
...
#
    ...
    assert len(params_enet_tvgn) == 99
    ...
    assert len(params) * len(cv) == 3888

    config = dict(data=dict(X="X_mci_only.npy", y="y_mci_only.npy"),
