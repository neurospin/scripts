...
class AlgorithmSnapshot:
    ...
    def save_nipals(self,locals):
        self.cpt += 1
        snapshot = dict(func = locals["_func"],v_func=locals["_v_func"],func_no_loss = locals["_v_func_no_loss"])
        cpt_str = str(self.cpt)
        output_filename = self.output_prefix + 'component:%s.npz' % (cpt_str)
        np.savez_compressed(output_filename, **snapshot)
