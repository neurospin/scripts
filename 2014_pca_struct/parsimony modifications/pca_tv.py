...
class RightSingularL1L2SmoothedTV_CONESTA(properties.CompositeFunction,
    ...
    def set_params(self, **kwargs):
        ...
        super(PCA_L1_L2_TV, self).set_params(**kwargs)
    ...
    def f_no_loss(self, beta):
        """Function value.
        """
        return self.l2p.f(beta) + \
            self.l1.f(beta) + \
            self.tv.f(beta)
    ...
    def mu_opt(self, eps):
        ...
        gA2 = self.tv.L(self.lmaxA)  # Gamma is in here!
    ...
    def eps_opt(self, mu):
        ...
        # factored out in this solution.
        old_mu = self.tv.set_mu(1.0)
        gA2 = self.tv.L(lmaxA)  # Gamma is in here!
        self.tv.set_mu(old_mu)
...
class PCA_L1_L2_TV(BaseEstimator):
    ...
    def __init__(self, l1, l2, ltv, Atv, n_components, mu=None,
                 ...
                 raise_if_l1_too_large=True,callback=None):
    ...
    def fit(self, X, in_place=False):
        ...
        for j in range(self.n_components):
            ...
            while (not _converged) and (not _stopped):
                ...
                info_conf = [Info.fvalue, Info.num_iter,Info.func_val,Info.num_iter]
                function = RightSingularL1L2SmoothedTV_CONESTA(X,
                                                               u_new,
                                                               self.l1,
                                                               self.l2,
                                                               self.ltv,
                                                               Atv=self.Atv,lmaxA=self.lmaxA)
                ...
                _v_func_no_loss.append(function.f_no_loss(v_new))
                if self.output:
...
#                    X_approx = d * np.dot(u_new, v_new.T)
            ...
            self.func_no_loss.append(_v_func_no_loss)
            #print (self.func_no_loss + self.func + self.v_func)
...
if __name__ == '__main__':
    ...
    numpy.random.seed(seed=13031981)
    ...
    X = sklearn.preprocessing.scale(X, with_mean=True, with_std=False)
     ...
     # A matrices
    Atv, n_compacts = nesterov.tv.linear_operator_from_shape(natural_shape)
    ...
    u /= np.linalg.norm(u)
    f = RightSingularL1L2SmoothedTV_CONESTA(X, u, l1, l2, ltv, Atv)

    # Fit an estimator without l1 and TV constraints and compare to PCA
    e_con = PCA_L1_L2_TV(l1=0.0,
                         l2=1.0,
                         ltv=6e-8, Atv=Atv,
                         n_components=2,
                         output=True)
