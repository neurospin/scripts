...
for directory in glob.glob(os.path.join(path,'*')):
    if os.path.isdir(directory) and count >= 0:
        ...
        for j in range(len(areas)):
            if D[j] != 0:
                E[:,j] = C[:,j]/D[j]
        connectivity_matrix[count,:] = E.flatten()
        count += 1
elapsed = time.time() - t
...
for j in range(nb_subjects):
    scatter_matrix += (connectivity_matrix[j,:].reshape(len(areas)*len(areas),1) - means).dot((connectivity_matrix[j,:].reshape(len(areas)*len(areas),1) - means).T)
    print "Subject number: "+str(j)

# Now just load the result
#scatter_matrix = np.loadtxt('/neurospin/brainomics/2016_connectogram/scatter_matrix.txt')
...
#eig_val_sc = np.loadtxt('/neurospin/brainomics/2016_connectogram/eig_val_sc.txt').view(complex)
#eig_vec_sc = np.loadtxt('/neurospin/brainomics/2016_connectogram/eig_vec_sc.txt').view(complex)
elapsed = time.time() - t
