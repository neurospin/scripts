...
for i in range(50):
    ...
    frob_test[i,3] = score_cv_enettv.frobenius_test
    ...
    frob_train[i,3] = score_cv_enettv.frobenius_train
    ...
    MSE_results[i,3] = compute_mse(p = best_enettv_param,beta_star_path= INPUT_BETA_FILE,beta_path= INPUT_RESULTS_DIR)
...
# TV vs sparse
tval, pval = scipy.stats.ttest_rel(frob_test [:,1],frob_test [:,3], axis=0)
print ("Frobenius stats for TV vs sparse: T = %r , pvalue = %r ") %(tval, pval)

#TV vs Enet
tval, pval = scipy.stats.ttest_rel(frob_test [:,2],frob_test [:,3], axis=0)
