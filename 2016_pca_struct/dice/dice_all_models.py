...
def reducer(key, values):
    ...
    paths = [p for p in paths if not p.count("struct_pca_0.1_1e-06_0.1")]
    ...
    paths = [p for p in paths if not p.count("struct_pca_0.1_0.1_0.5")]
    paths = [p for p in paths if not p.count("struct_pca_0.1_0.5_0.5")]
    paths = [p for p in paths if not p.count("sparse_pca_0.0_0.0_10.0")]
    paths = [p for p in paths if not p.count("struct_pca_0.1_0.5_0.1")]
...
#        # Create files to synchronize with the cluster
        ...
        # Create job files
        cluster_cmd = "mapreduce.py -m %s/config_dCV.json  --ncore 20" % CLUSTER_WD
        clust_utils.gabriel_make_qsub_job_files(output_dir, cluster_cmd,walltime= "1000:00:00")
