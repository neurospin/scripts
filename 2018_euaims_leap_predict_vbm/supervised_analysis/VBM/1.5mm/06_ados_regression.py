...
WD = '/neurospin/brainomics/2018_euaims_leap_predict_vbm/results/VBM/ados_regression'
WD_CLUSTER = WD.replace("/neurospin/", "/mnt/neurospin/sel-poivre/")

DATA_PATH = '/neurospin/brainomics/2018_euaims_leap_predict_vbm/results/VBM/data'
user_func_filename = '/home/ad247405/git/scripts/2018_euaims_leap_predict_vbm/supervised_analysis/VBM/06_ados_regression.py'
...
penalty_start = 3 #age gender and intercept
##############################################################################
def init():
    os.makedirs(WD, exist_ok=True)
    shutil.copy(os.path.join(DATA_PATH, 'X.npy'), WD)
    shutil.copy(os.path.join(DATA_PATH, 'ados_tot.npy'), WD)
    # VBM
    if DATA_TYPE == "image":
    ...
    os.chdir(WD)
    X = np.load("X.npy")
    y = np.load("ados_tot.npy")

    Z = np.ones((y.shape[0], 1)) # intercept
    X = np.hstack([Z, X])

    X = X[y!=999,:]
    Z = Z[y!=999]
    y = y[y!=999]
    assert X.shape == (258, 240058)
    np.save(os.path.join(WD, 'X.npy'),X)
    np.save(os.path.join(WD, 'ados_tot.npy'),y)
...
#
    ...
    config = dict(data=dict(X="X.npy", y="ados_tot.npy"),
