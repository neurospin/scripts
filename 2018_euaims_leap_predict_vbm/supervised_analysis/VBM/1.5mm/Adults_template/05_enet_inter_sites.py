...
WD = '/neurospin/brainomics/2018_euaims_leap_predict_vbm/results/VBM/1.5mm/Adult_Template/enet_inter_sites'
WD_CLUSTER = WD.replace("/neurospin/", "/mnt/neurospin/sel-poivre/")

DATA_PATH = '/neurospin/brainomics/2018_euaims_leap_predict_vbm/results/VBM/1.5mm/Adult_Template/data'
user_func_filename = '/home/ad247405/git/scripts/2018_euaims_leap_predict_vbm/supervised_analysis/VBM/1.5mm/Adults_template/05_enet_inter_sites.py'

def config_filename(): return os.path.join(WD,"config_dCV.json")
def results_filename(): return os.path.join(WD,os.path.basename(WD) + "_dcv.xlsx")

NFOLDS_OUTER = 6 #number of sites
NFOLDS_INNER = 5
penalty_start = 2
#DATA_TYPE = "mesh"
DATA_TYPE = "image"
...
def init():
    ...
    site = np.load("/neurospin/brainomics/2018_euaims_leap_predict_vbm/results/VBM/1.5mm/Adult_Template/data/site.npy")
    ...
    cv_outer[5][0] = np.transpose(np.where(site != 6)).ravel()
    cv_outer[5][1] = np.transpose(np.where(site == 6)).ravel()
    ...
    clust_utils.gabriel_make_qsub_job_files(WD, cmd,walltime = "350:00:00",
...
def reducer(key=None, values=None):
    ...
    def argmaxscore_bygroup(data, groupby='fold', param_key="key",
        ...
        if refit_key is not None:
            refit = score_func(refit_key,
                                    [os.path.join(config['map_output'], row[groupby], "all", row[param_key])
    ...
    paths_all = glob.glob("results/cv0?/all/*")
    paths_all.sort()
    ...
    print('## -----------------------------------------')
    paths = glob.glob("results/cv0?/cvnested0?/*")
    paths.sort()
    ...
    assert l1l2_reduced.shape[0] == 16
    ...
    assert l1l2_ridge_reduced.shape[0] == 8
    ...
    assert l1l2_lasso_reduced.shape[0] == 8
