# -*- coding: utf-8 -*-
"""
Created on Wed Dec 17 14:08:51 2014
...
INPUT_CSV = os.path.join(BASE_PATH,      "ADRS_population",
...
np.save(os.path.join(OUTPUT_DATA, "y_discrete-adrs.npy"), y)
...
mask_atlas_arr = brainomics.image_atlas.smooth_labels(mask_atlas_arr,
                                                      size=(3, 3, 3))
assert np.sum(mask_atlas_arr != 0) == 627176
out_im = nib.Nifti1Image(mask_atlas_arr,
                             affine=babel_image.get_affine())
out_im.to_filename(os.path.join(OUTPUT_DATA,
                                "mask_atlas_discrete-adrs.nii.gz"))
im = nib.load(os.path.join(OUTPUT_DATA, "mask_atlas_discrete-adrs.nii.gz"))
...
print "mask sum4: ", mask_atlas_binarized_arr.sum()
assert mask_atlas_binarized_arr.sum() == 627176
out_im = nib.Nifti1Image(mask_atlas_binarized_arr.astype("int16"),
                             affine=babel_image.get_affine())
out_im.to_filename(os.path.join(OUTPUT_DATA,
                                "mask_atlas_binarized_discrete-adrs.nii.gz"))
babel_mask = nib.load(os.path.join(OUTPUT_DATA,
                                "mask_atlas_binarized_discrete-adrs.nii.gz"))
...
assert X.shape == (618, 627179)
n, p = X.shape
X -= X.mean(axis=0)
X /= X.std(axis=0)
X[:, 0] = 1.
np.save(os.path.join(OUTPUT_DATA, "X_discrete-adrs.npy"), X)
fh = open(os.path.join(OUTPUT_DATA,
                       "X_discrete-adrs.npy").replace("npy", "txt"), "w")
