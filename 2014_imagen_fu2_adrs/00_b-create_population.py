...
INPUT_CLINIC_FILENAME = os.path.join(BASE_PATH, "clinic",
                                     "imagen_adrs_fu1_20140528.csv")
INPUT_SUBJECTS_LIST_FILENAME = "/neurospin/brainomics/2012_imagen_shfj\
                                        /VBM/new_segment_spm8/smwc1*"
...
#Psytool QC
#The Psytools Valid flag is shown as ‘invalid’ if at least one of the
...
input_subjects = [int(regexp.findall(os.path.basename(x))[0]) \
                            for x in input_subjects_filenames]

subjects_images = pd.DataFrame(dict(Subject=input_subjects,
                                    mri_path=input_subjects_filenames))
