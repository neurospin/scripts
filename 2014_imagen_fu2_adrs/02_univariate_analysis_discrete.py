# -*- coding: utf-8 -*-
"""
Created on Wed Dec 17 14:11:00 2014
...
mask_ima = nib.load(os.path.join(DATA_PATH,
                                 "mask_atlas_binarized_discrete-adrs.nii.gz"))
mask_arr = mask_ima.get_data() != 0

X = np.load(os.path.join(DATA_PATH, "X_discrete-adrs.npy"))

y = np.load(os.path.join(DATA_PATH, "y_discrete-adrs.npy"))
Z = X[:, :3]
Y = X[:, 3:]
del X
...
out_im.to_filename(os.path.join(OUTPUT_UNIVARIATE, "pval_discrete-adrs.nii.gz"))

p_log10 = - np.log10(pvals)
arr = np.zeros(mask_arr.shape)
arr[mask_arr] = p_log10[0]
out_im = nib.Nifti1Image(arr, affine=mask_ima.get_affine())
out_im.to_filename(os.path.join(OUTPUT_UNIVARIATE, "pval_-log10_discrete-adrs.nii.gz"))

arr = np.zeros(mask_arr.shape)
arr[mask_arr] = tvals[0]
out_im = nib.Nifti1Image(arr, affine=mask_ima.get_affine())
out_im.to_filename(os.path.join(OUTPUT_UNIVARIATE, "tstat_discrete-adrs.nii.gz"))

X = np.load(os.path.join(DATA_PATH, "X_discrete-adrs.npy"))
import matplotlib.pyplot as plt
plt.plot(y, X[:, np.argmax(tvals)], "ob")
plt.savefig(os.path.join(OUTPUT_UNIVARIATE, "gm-where-maxtval_x_discrte-adrs.svg"))
...
#               "tstat_discrete-adrs.nii.gz"))
#
#arr = np.zeros(mask_arr.shape)
#arr[mask_arr] = pvals_perm[0]
#out_im = nib.Nifti1Image(arr, affine=mask_ima.get_affine())
#out_im.to_filename(os.path.join(OUTPUT_UNIVARIATE,
#         "ptmax_discrete-adrs.nii.gz"))
#
...
#out_im = nib.Nifti1Image(arr, affine=mask_ima.get_affine())
#out_im.to_filename(os.path.join(OUTPUT_UNIVARIATE,
#         "ptmax-log10_discrete-adrs.nii.gz"))
