# -*- coding: utf-8 -*-
"""
Created on Fri Oct  3 13:44:37 2014
...
(04-a_multivariate_1627SNPs_BMI.py), so that we can run the model on the
whole dataset.
"""
...
if __name__ == "__main__":
    ...
    # hyperparameters
    X, z, SNPs = load_SNPs_bmi_data(cache=False)

    # Get variables' names
    variables = np.hstack((SNPs.columns))
    ...
    beta_SNPs_BMI_assoc_path = os.path.join(BASE_PATH, 'results',
                                            'beta_1627SNPs_BMI_assoc.txt')
    with open(beta_SNPs_BMI_assoc_path, 'wb') as csvfile:
