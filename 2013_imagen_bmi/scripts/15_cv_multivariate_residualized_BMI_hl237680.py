...
if __name__ == "__main__":
    ...
    ## Set pathes
    WD = "/neurospin/tmp/brainomics/residual_bmi_images_cluster-8"
    ...
    params = [[alpha, l1_ratio] for alpha in np.arange(0.007, 0.025, 0.013) for l1_ratio in np.arange(0.1, 1., .2)]
    ...
    user_func_filename = os.path.join("/home/hl237680",
        ...
        "15_cv_multivariate_residualized_BMI_hl237680.py")
    ...
    config = dict(data=dict(X='X.npy', z='z.npy'),
                  ...
                  reduce_output="results-8.csv")
    json.dump(config, open(os.path.join(WD, "config-8.json"), "w"))
    ...
    cmd = "mapreduce.py -m %s/config-8.json  --ncore 12" % WD_CLUSTER
    ...
    print "mapreduce.py -m %s/config-8.json --ncore 12" % WD
    ...
    #############################################################################
    print "# Reduce"
    print "mapreduce.py -r %s/config-8.json" % WD
