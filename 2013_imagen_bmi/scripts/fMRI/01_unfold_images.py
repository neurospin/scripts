# -*- coding: utf-8 -*-
"""
Created on Fri Sep 26 09:05:00 2014

@author: hl237680

Read activation maps from fMRI acquisitions, unfold them for each subject in
order to get an array where each row corresponds to a subject and each column
to the different voxels within the subject's image.
...
- '/neurospin/brainomics/2013_imagen_bmi/data/fMRI_GCA_motor_left/
    GCA_motor_left_images.hdf5'
    fMRI images dumped into an hdf5 file
...
OUTPUT_DIR = os.path.join(DATA_PATH, 'GCA_motor_left')
if not os.path.exists(OUTPUT_DIR):
    os.makedirs(OUTPUT_DIR)

OUTPUT_FILE = os.path.join(OUTPUT_DIR, 'GCA_motor_left_images.npy')
OUTPUT_MASK = os.path.join(OUTPUT_DIR, 'GCA_motor_left_mask.nii')
