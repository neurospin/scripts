...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec  2 11:23:26 2016
...
INPUT_CSV = "/neurospin/brainomics/2016_deptms/analysis/VBM/results/enettv/model_selection_5folds/results_dCV_5folds.xlsx"
...
d1=data[np.round((data.l1)/(data.l2+ data.l1 ),3) == 0.01]
d2=data[np.round((data.l1)/(data.l2+ data.l1),3) == 0.1]
d3=data[np.round((data.l1)/(data.l2+ data.l1),3) == 0.5]
d4=data[np.round((data.l1)/(data.l2+ data.l1),3) == 1.0]

        
#d1=data[data.l1_ratio == 0.01]
#d2=data[data.l1_ratio == 0.10]
#d3=data[data.l1_ratio == 0.50]
#d3=data[data.l1_ratio == 1.0]
#
...
plt.plot(d3.tv, d3.recall_mean,"blue",label=r'$\lambda_1/(\lambda_1 + \lambda_2)= 0.50 $',linewidth=2)
...
plt.title('3D voxel-based GM map - $alpha = 0.01$')
