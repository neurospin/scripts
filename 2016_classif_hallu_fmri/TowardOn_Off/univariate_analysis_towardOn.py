# -*- coding: utf-8 -*-
"""
Created on Wed Dec  9 13:19:16 2015
@author: ad247405
...
for i in range(1,31):
    ...
    #keep only off and on scans for further analysis
        ...
        p=max(int(nperiods_off),int(nperiods_transi))
        
        DesignMat = np.zeros((X.shape[0],nperiods_off + nperiods_transi)) # 
        contrasts=np.zeros((nperiods_off+nperiods_transi))
        k=0
        curr_pos=0
...
out_im.to_filename(os.path.join(BASE_PATH,"toward_on","univariate","tval.nii.gz"))
...
out_im = nibabel.Nifti1Image(arr, affine=babel_mask.get_affine())
out_im.to_filename(os.path.join(BASE_PATH,"toward_on","univariate","pval.nii.gz"))
...
out_im = nibabel.Nifti1Image(arr, affine=babel_mask.get_affine())
out_im.to_filename(os.path.join(BASE_PATH,"toward_on","univariate","log10_p.nii.gz"))
...
out_im = nibabel.Nifti1Image(arr, affine=babel_mask.get_affine())
out_im.to_filename(os.path.join(BASE_PATH,"toward_on","univariate","corr_pval.nii.gz"))
...
arr[mask_bool] = log10_ps
out_im = nibabel.Nifti1Image(arr, affine=babel_mask.get_affine())
out_im.to_filename(os.path.join(BASE_PATH,"toward_on","univariate","log10_corr_pval.nii.gz"))
