# -*- coding: utf-8 -*-
"""
Created on Wed Nov 16 17:03:05 2016
...
INPUT_CSV = os.path.join(BASE_PATH,"results","Freesurfer","ICAAR+EUGEI","population.csv")
OUTPUT = os.path.join(BASE_PATH,"results","Freesurfer","ICAAR+EUGEI","data")
...
assert Xtot.shape == (75, 327684)

mask = ((np.max(Xtot, axis=0) > 0) & (np.std(Xtot, axis=0) > 1e-2))
assert mask.sum() ==  316595

np.save(os.path.join(OUTPUT, "mask.npy"), mask)

X = Xtot[:, mask]
assert X.shape == (75,316595)

#############################################################################

X = np.hstack([Z, X])
assert X.shape == (75, 316597)
