...
babel_mask_atlas = brainomics.image_atlas.resample_atlas_harvard_oxford(
    ...
    output=os.path.join(OUTPUT_DATA, "mask_biomarkers.nii"))
...
assert np.sum(mask_atlas != 0) == 25103
out_im = nibabel.Nifti1Image(mask_atlas,
                             affine=babel_image.get_affine())
out_im.to_filename(os.path.join(OUTPUT_DATA, "mask_biomarkers.nii"))
im = nibabel.load(os.path.join(OUTPUT_DATA, "mask_biomarkers.nii"))
...
out_im = nibabel.Nifti1Image(mask_bool.astype("int16"),
                             affine=babel_image.get_affine())
out_im.to_filename(os.path.join(OUTPUT_DATA, "mask_biomarkers.nii"))
babel_mask = nibabel.load(os.path.join(OUTPUT_DATA, "mask_biomarkers.nii"))
...
X = np.hstack([Z, X])
assert X.shape == (75, 25107)

#Remove nan lines 
X= X[np.logical_not(np.isnan(y)).ravel(),:]
y=y[np.logical_not(np.isnan(y))]
assert X.shape == (59, 25107)
...
n, p = X.shape
X = np.nan_to_num(X)
np.save(os.path.join(OUTPUT_DATA, "X_masked.npy"), X)
