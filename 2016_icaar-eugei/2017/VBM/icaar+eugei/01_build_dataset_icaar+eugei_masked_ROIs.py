...
OUTPUT_DATA = os.path.join(BASE_PATH,"VBM","ICAAR+EUGEI","data","data_atlas_subcort")
...
babel_mask_atlas = brainomics.image_atlas.resample_atlas_harvard_oxford(
    ...
    output=os.path.join(OUTPUT_DATA, "mask_atlas_sub.nii"))
...
assert np.sum(mask_atlas != 0) == 16021
out_im = nibabel.Nifti1Image(mask_atlas,
                             affine=babel_image.get_affine())
out_im.to_filename(os.path.join(OUTPUT_DATA, "mask_atlas_sub.nii"))
im = nibabel.load(os.path.join(OUTPUT_DATA, "mask_atlas_sub.nii"))
...
out_im = nibabel.Nifti1Image(mask_bool.astype("int16"),
                             affine=babel_image.get_affine())
out_im.to_filename(os.path.join(OUTPUT_DATA, "mask_atlas_sub.nii"))
babel_mask = nibabel.load(os.path.join(OUTPUT_DATA, "mask_atlas_sub.nii"))
...
X = np.hstack([Z, X])
assert X.shape == (75, 16025)

#Remove nan lines 
X= X[np.logical_not(np.isnan(y)).ravel(),:]
y=y[np.logical_not(np.isnan(y))]
assert X.shape == (59, 16025)
...
n, p = X.shape
X = np.nan_to_num(X)
np.save(os.path.join(OUTPUT_DATA, "X_masked_atlas.npy"), X)
