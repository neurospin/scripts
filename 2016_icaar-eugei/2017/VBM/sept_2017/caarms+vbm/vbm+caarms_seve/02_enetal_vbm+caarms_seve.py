...
WD = '/neurospin/brainomics/2016_icaar-eugei/2017_icaar_eugei/VBM/ICAAR/results/CAARMS+VBM/vbm+caarms_seve/enetal'
WD_CLUSTER = WD.replace("/neurospin/", "/mnt/neurospin/sel-poivre/")

DATA_PATH = '/neurospin/brainomics/2016_icaar-eugei/2017_icaar_eugei/VBM/ICAAR/data/data_vbm+caarms'
user_func_filename = '/home/ad247405/git/scripts/2016_icaar-eugei/2017/VBM/sept_2017/caarms+vbm/vbm+caarms_seve/02_enetal_vbm+caarms_seve.py'
...
def init():
    ...
    INPUT_X = os.path.join(DATA_PATH,'X_vbm+caarms_seve.npy')
    shutil.copy(INPUT_X, WD)
    shutil.copy(os.path.join(DATA_PATH, 'y.npy'), WD)

    # VBM
    if DATA_TYPE == "image":
        shutil.copy(os.path.join(DATA_PATH, 'mask_whole_brain.nii'), WD)
    elif DATA_TYPE == "mesh":
    ...
    ## Create config file
    os.chdir(WD)
    X = np.load(os.path.basename(INPUT_X))
    y = np.load("y.npy")
...
#
    ...
    config = dict(data=dict(X=os.path.basename(INPUT_X), y="y.npy"),
