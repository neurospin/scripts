...
% SubjID: a list of subject IDs %% Yann's correct mistake here
...
% read mask files
fid = fopen([FSDir, 'fsaverage/label/lh.cortex.label']);   % open mask file for the lh
...
fid = fopen([FSDir, 'fsaverage/label/rh.cortex.label']);   % open mask file for the rh
...
for i = 1:Nsubj
    FileLh = [SurfDir,'lh.curv.pial/', strcat(SubjID{i},ImgFileLh)];   % file name
    ...
    FileRh = [SurfDir, 'rh.curv.pial/',strcat(SubjID{i}, ImgFileRh)];   % file name
