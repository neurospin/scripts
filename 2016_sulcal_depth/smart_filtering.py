...
DIRECTORY_PHENO = DIRECTORY_STAP+'new_pheno/'
...
def plot_scatter(x,y):
    ...
    circle1 = plt.Circle((np.mean(x),np.mean(y)), radius=nb_stds*abs(np.std(x)+np.std(y)), color='g') 
    p = np.polyfit(x, y, 1)
    print p
...
x = df_right2['geodesicDepthMax']
y = df_left2['geodesicDepthMax']
plot_scatter(x,y)
...
mu_x = np.mean(x)
mu_y = np.mean(y)
radius=nb_stds*abs(np.std(x)+np.std(y))
x_norm = x-mu_x
y_norm = y-mu_y
