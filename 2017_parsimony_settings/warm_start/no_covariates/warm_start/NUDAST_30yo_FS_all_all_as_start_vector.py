...
WD = '/neurospin/brainomics/2017_parsimony_settings/warm_restart/NUSDAST_30yo/FS/no_covariates/warm_restart/all_all_as_start_vector'
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(WD, "beta_start.npy")):
        betas = dict()
        BETA_START_PATH = "/neurospin/brainomics/2017_parsimony_settings/warm_restart/NUSDAST_30yo/FS/no_covariates/no_warm_restart/model_selectionCV/all/all"
        params = glob.glob(os.path.join( BETA_START_PATH,"0*"))
        for p in params:
            print(p)
            path = os.path.join(p,"beta.npz")
            beta = np.load(path)
            betas[os.path.basename(p)] = beta['arr_0'][penalty_start:,:]
...
#
    ...
    user_func_filename = "/home/ad247405/git/scripts/2017_parsimony_settings/warm_start/no_covariates/warm_start/NUDAST_30yo_FS_all_all_as_start_vector.py"
