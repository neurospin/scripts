...
WD = '/neurospin/brainomics/2017_parsimony_settings/warm_restart/NUSDAST_30yo/VBM/warm_restart/cv04_all_as_start_vector'
...
def init():
    ...
    if not os.path.exists(os.path.join(WD, "beta_start.npz")):
        betas = dict()
        BETA_START_PATH = "/neurospin/brainomics/2017_parsimony_settings/warm_restart/NUSDAST_30yo/VBM/no_warm_restart/model_selectionCV/cv04/all"
        params = glob.glob(os.path.join( BETA_START_PATH,"0*"))
        for p in params:
            print(p)
            path = os.path.join(p,"beta.npy")
            beta = np.load(path)
            betas[os.path.basename(p)] = beta['arr_0']

        np.savez(os.path.join(WD, "beta_start.npy"),**betas)
...
#
    ...
    user_func_filename = "/home/ad247405/git/scripts/2017_parsimony_settings/warm_restart/NUDAST_30yo_VBM_cv04_all_as_start_vector.py"
...
def mapper(key, output_collector):
    ...
    beta_start = GLOBAL.BETA_START[os.path.basename(output_collector.output_dir)]
