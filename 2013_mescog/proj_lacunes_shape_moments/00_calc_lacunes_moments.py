...
base_dir = "/home/edouard/data/2013_mescog/images/originals/Munich/CAD_bioclinica_nifti"
...
files = glob.glob(os.path.join(base_dir, "*", "*LL.nii.gz"))

file_ll_path = files[0]

    cmd = 'AimsFileConvert --input %s --output %s' % \
    (big_clust_vol_filename, big_clust_graph_filename)
    lines = cmd)


os.popen('AimsFileConvert -i %s -t S16 -o /tmp/toto.nii' % file_ll_path)
os.popen('AimsConnectComp -i /tmp/toto.nii -o /tmp/toto.nii')

out = os.popen('AimsMoment -i /tmp/toto.nii')
lacune = None
...
for line in out:
    ...
    if item[0] == 'Results for label':
        ...
        lacune_id = int(item[1])
        Moment_invariant = False
    if len(item) == 1 and item[0] == '':
        lacune_cols, lacune_values = zip(*lacune_keys_values)
