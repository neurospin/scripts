...
write.csv(d, "/neurospin/mescog/proj_predict_cog_decline/20140728_nomissing_BPF-LLV_imputed/enet/M36/RESULTS_TAB_CV_SUMMARY.csv")
#d$GROUP = factor(attr(y_pred_rtree, "group"), levels=as.integer(names(label)), labels=label)

pcv = ggplot(d, aes(x = PREDICTORS, y = r2_te, fill=PREDICTORS)) +
