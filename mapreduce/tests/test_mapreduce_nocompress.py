...
Test optional non-compressed output.
"""
...
def mapper(key, output_collector):
    ...
    # We add test to test the serialization of arbitrary values
    output_collector.collect(key, dict(y_pred=y_pred, y_true=ytest, test=42))
...
if __name__ == "__main__":
    ...
    ###########################################################################
    ## Apply map
    map_cmd = "%s --raw -v --map %s/config.json" % (exec_path, WD)
