...
WD = 'linear_regression_sklearn_patients_MAASCtot'
...
user_func_filename = '/home/ad247405/git/scripts/2016_AUSZ/2017/sept_2017/VBM/regression_svm_enet_sklearn_patients_on_MAASCtot.py'
...
def init():
    ...
    shutil.copy(os.path.join(DATA_PATH, 'y_patients_only.npy'), WD)
    # VBM
    if DATA_TYPE == "image":
    ...
    DX = np.load("y_patients_only.npy")
    X = X[np.logical_not(np.isnan(y)).ravel(),:]
    y = y[np.logical_not(np.isnan(y))]
    assert X.shape == (80, 261212)
    np.save(os.path.join(DATA_PATH, 'X_patients_only.npy'),X)
    np.save(os.path.join(DATA_PATH, 'MASCtot_patients_only.npy'),y)
