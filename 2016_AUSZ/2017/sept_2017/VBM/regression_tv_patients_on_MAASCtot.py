...
WD = '/neurospin/brainomics/2016_AUSZ/september_2017/results/VBM/linear_regression_TV_patients_MAASCtot'
WD_CLUSTER = WD.replace("/neurospin/", "/mnt/neurospin/sel-poivre/")

DATA_PATH = '/neurospin/brainomics/2016_AUSZ/september_2017/results/VBM/data'
user_func_filename = '/home/ad247405/git/scripts/2016_AUSZ/2017/sept_2017/VBM/regression_tv_patients_on_MAASCtot.py'
...
def init():
    ...
    shutil.copy(os.path.join(DATA_PATH, 'X_patients_only.npy'), WD)
    shutil.copy(os.path.join(DATA_PATH, 'MASCtot_patients_only.npy'), WD)
    # VBM
    if DATA_TYPE == "image":
    ...
    ## Create config file
    os.chdir(WD)
    X = np.load("X_patients_only.npy")
    y = np.load("MASCtot_patients_only.npy")

    X = X[np.logical_not(np.isnan(y)).ravel(),:]
    y = y[np.logical_not(np.isnan(y))]
    assert X.shape == (80, 261210)
    np.save(os.path.join(WD, 'X_patients_only.npy'),X)
    np.save(os.path.join(WD, 'MASCtot_patients_only.npy'),y)
...
#
    ...
    assert len(params) * len(cv) == 9936


    config = dict(data=dict(X="X_patients_only.npy", y="MASCtot_patients_only.npy"),
...
def mapper(key, output_collector):
    ...
    if algo == 'enettv':
        ...
        mod = estimators.RidgeRegression(l1l2ratio,algorithm=gradient.GradientDescent(max_iter=1000))
        mod.fit(Xtr, ytr.ravel())
        beta=mod.beta

    elif algo == 'RidgeMeanFalse':
        mod = estimators.RidgeRegression(l1l2ratio,mean=False)
        mod.fit(Xtr, ytr.ravel())
        beta=mod.beta
        ...
        mod = linear_model.Ridge(alpha = l1l2ratio)
        mod.fit(Xtr, ytr.ravel())
        beta=mod.coef_
        beta = beta.reshape(beta.shape[0],1)
    else:
