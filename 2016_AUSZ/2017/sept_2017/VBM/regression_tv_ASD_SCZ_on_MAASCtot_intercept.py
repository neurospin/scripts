...
WD = '/neurospin/brainomics/2016_AUSZ/september_2017/results/VBM/linear_regression_TV_ASD_SCZ_MAASCtot_intercept_penalty_start'
WD_CLUSTER = WD.replace("/neurospin/", "/mnt/neurospin/sel-poivre/")

DATA_PATH = '/neurospin/brainomics/2016_AUSZ/september_2017/results/VBM/data/data_with_intercept/data_ASD_SCZ_only'
user_func_filename = '/home/ad247405/git/scripts/2016_AUSZ/2017/sept_2017/VBM/regression_tv_ASD_SCZ_on_MAASCtot_intercept.py'
...
def init():
    ...
    shutil.copy(os.path.join(DATA_PATH, 'MASCtot.npy'), WD)
    # VBM
    if DATA_TYPE == "image":
    ...
    os.chdir(WD)
    X = np.load("X.npy")
    y = np.load("MASCtot.npy")

    X = X[np.logical_not(np.isnan(y)).ravel(),:]
    y = y[np.logical_not(np.isnan(y))]
    assert X.shape == (62, 261213)
    np.save(os.path.join(WD, 'X.npy'),X)
    np.save(os.path.join(WD, 'MASCtot.npy'),y)
...
#
    ...
    config = dict(data=dict(X="X.npy", y="MASCtot.npy"),
