...
if __name__ == "__main__":
    ...
    tol = 0.02
    path = '/volatile/yann/2015_asym_sts/all_pheno0.02_GCTA/hsq_files_left_handed'
    ...
    for j in range(len(pheno)):
        ...
        for filename in glob.glob(os.path.join(path,'pruned2Yann*.phe_'+pheno[j]+'_depthMax.hsq')):
            #print filename
            m = re.search('pruned2Yann_m0.01_g1_h6_wsi50_wsk5_vif10.0_Covcovar_GenCitHan_GCTA.cov_QcovAgeIBS.qcovar_Phe(.+?).phe_'+pheno[j]+'_depthMax.hsq', filename)
            if m:
                sulcus = m.group(1)
        ...
        index_p_values = np.nonzero(np.less(p_values, 5e-2))[0]
