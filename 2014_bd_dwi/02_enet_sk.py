...
create config file for FA images skeletonised. 
"""
...
# CS with Intercept
    ...
    WD = "/volatile/share/2014_bd_dwi/bd_dwi_enettv_csi_sk"
...
# CS without Intercept
    ...
    WD = "/volatile/share/2014_bd_dwi/bd_dwi_enettv_cs_sk"
    key = '_'.join(str(p) for p in config['params'][0])
    #class GLOBAL: DATA = dict()
    load_globals(config)
    OUTPUT = os.path.join(WD, 'test', key)
...
if __name__ == "__main__":
    ...
    # Directory
    INPUT_DIR_CSI = "/volatile/share/2014_bd_dwi/bd_dwi_csi_sk"
    WD_CSI = "/volatile/share/2014_bd_dwi/bd_dwi_enettv_csi_sk"

    INPUT_DIR_CS = "/volatile/share/2014_bd_dwi/bd_dwi_cs_sk"
    WD_CS = "/volatile/share/2014_bd_dwi/bd_dwi_enettv_cs_sk"
    ...
    config = dict(data=dict(X=INPUT_DATA_X, y=INPUT_DATA_y),
                  ...
                  map_output="results_CSI_sk",
                  user_func=user_func_filename,
                  reduce_input="results_CSI_sk/*/*",
                  reduce_group_by="results_CSI_sk/.*/(.*)",
                  reduce_output="results_CSI_sk.csv")
    json.dump(config, open(os.path.join(WD_CSI, "config_CSI_sk.json"), "w"))

    # Utils files with intercept
    sync_push_filename, sync_pull_filename, WD_CLUSTER = \
        clust_utils.gabriel_make_sync_data_files(WD_CSI, user="md238665")
    cmd = "mapreduce.py --map  %s/config_CSI_sk.json" % WD_CLUSTER
    ...
    config = dict(data=dict(X=INPUT_DATA_X, y=INPUT_DATA_y),
                  ...
                  map_output="results_C_sk",
                  user_func=user_func_filename,
                  reduce_input="results_CS_sk/*/*",
                  reduce_group_by="results_CS_sk/.*/(.*)",
                  reduce_output="results_CS_sk.csv")
    json.dump(config, open(os.path.join(WD_CS, "config_CS_sk.json"), "w"))

    # Utils files without intercept
    sync_push_filename, sync_pull_filename, WD_CLUSTER = \
        clust_utils.gabriel_make_sync_data_files(WD_CS, user="md238665")
    cmd = "mapreduce.py --map  %s/config_CS_sk.json" % WD_CLUSTER
