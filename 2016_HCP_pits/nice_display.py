...
if __name__ == '__main__':
    ...
    INPUT = '/home/yl247234/Images/final_snap/group_'+database_parcel+'_'+base+'/'
    ...
    OUTPUT = '/home/yl247234/Images/final_snap/results/'
    if not os.path.exists(OUTPUT):
        os.makedirs(OUTPUT)
    output_image = OUTPUT+database_parcel+'_'+base+'_poster.jpg'
    images = []
    features = ['density', 'density_smooth', 'clusters', 'density_thresholded']
    sides = ['L', 'R']
    view = ['extern', 'intern']
    for feature in features:
        for side in sides:
            for sd in view:
                ...
                images.append(image)
    features = ['DPF', 'sulc']#, 'case_control'] 
    sides = ['Left', 'Right']
    for feature in features:
        for side in sides:
            for sd in view:
    ...
    colorbar = colorbar.resize((int(colorbar.size[0]), int(colorbar.size[1])))
    padding = 4
    bgcolor = (255,255,255)
    ...
    features = ['Density single pits', 'Density smoothed', 'Clusters', 'After auto-thresholding', 'Depth', 'Sulc Depth']#, 'Case/Control']
    max_length = 0
    for feature in features:
