...
def plot_histo(ax, x):
    ...
    for n_components in range(1,3):
        ...
        gmm = mixture.GMM(n_components=n_components) # gmm for two components
        gmm.fit(X) # train it!
        bic.append(gmm.bic(X))
        if bic[-1] < lowest_bic:
            lowest_bic = bic[-1]
            best_gmm = gmm
            print "Best gmm with components "+str(n_components)+" and BIC "+str(bic[-1])
    gmm = best_gmm
...
if __name__ == '__main__': 
    ...
    for side in sides:
        ...
        ax = []
        for k,parcel in enumerate(parcels_org):
            #if int(parcel) in df_labels.index:
            if count == 30:
                count = 0
                gs = gridspec.GridSpec(5, 6, width_ratios=[8,8,8,8,8,8], height_ratios=[6,6,6,6,6])
                fig = plt.figure()
