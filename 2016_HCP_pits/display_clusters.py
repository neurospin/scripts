...
if "Freesurfer" in path0:
    INPUT  = '/neurospin/brainomics/2016_HCP/new_distrib/distribution_DPF_'+database_parcel+'_Freesurfer_new/'
else:
    INPUT  = '/neurospin/brainomics/2016_HCP/new_distrib/distribution_DPF_'+database_parcel+'_BV/'
...
for side in sides:
    ...
    if (side == "R" and "Freesurfer" not in path0) or (side == "L" and "Freesurfer" in path0):
...
for side in sides:
    ...
    meshes[side] = ana.loadObject(template_mesh)
    windows[side] = ana.createWindow('3D', geometry=[0, 0, 584, 584])
    file_parcels_on_atlas = path_parcels +'pits_density_update/'+side+'_average_pits_smoothed0.7_60.gii'
    parcels[side] = ana.loadObject(file_parcels_on_atlas)
