...
INPUT_CSV = os.path.join(BASE_PATH,          "ADAS11-MCIc-CTL", "population.csv")

OUTPUT = os.path.join(BASE_PATH,             "ADAS11-MCIc-CTL")
#OUTPUT_CS = os.path.join(BASE_PATH,          "ADAS11-MCIc-CTL_cs")
#OUTPUT_ATLAS = os.path.join(BASE_PATH,       "ADAS11-MCIc-CTL_gtvenet")
#OUTPUT_CS_ATLAS = os.path.join(BASE_PATH,    "ADAS11-MCIc-CTL_cs_gtvenet")

if not os.path.exists(OUTPUT): os.makedirs(OUTPUT)
#ADAS11-AD-CTLif not os.path.exists(OUTPUT_CS): os.makedirs(OUTPUT_CS)
...
n = len(pop)
assert n == 199
Z = np.zeros((n, 3)) # Age + Gender + EDUCATION
#Z[:, 0] = 1 # Intercept
y = np.zeros((n, 1)) # DX
images = list()
for i, PTID in enumerate(pop['PTID']):
    ...
    Z[i, :] = np.asarray(cur[["AGE", "PTGENDER.num", "PTEDUCAT"]]).ravel()
    y[i, 0] = cur["ADAS11.d800"]
...
assert np.sum(mask_atlas != 0) == 286214
...
X = np.hstack([Z, X])
assert X.shape == (199, 286214+3)
X -= X.mean(axis=0)
X /= X.std(axis=0)
n, p = X.shape
np.save(os.path.join(OUTPUT, "X.npy"), X)
fh = open(os.path.join(OUTPUT, "X.npy").replace("npy", "txt"), "w")
fh.write('Centered and scaled data. Shape = (%i, %i): Age + Gender + EDUCATION + %i voxels' % \
    (n, p, mask_bool.sum()))
fh.close()
