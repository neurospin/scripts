...
mkdir /neurospin/brainomics/2013_adni/MCIc-CTL-FS_cs_s_modselectcv
cd /neurospin/brainomics/2013_adni/MCIc-CTL-FS_cs_s_modselectcv

cp ../MCIc-CTL-FS_cs_s/X* .
cp ../MCIc-CTL-FS_cs_s/y.npy .
cp ../MCIc-CTL-FS_cs_s/mask.npy .
cp ../MCIc-CTL-FS_cs_s/lrh.pial.gii .
cp ../MCIc-CTL-FS_cs_modselectcv/config_modselectcv.json .

# Start by running Locally with 2 cores, to check that everything os OK)
Interrupt after a while CTL-C
mapreduce.py --map /neurospin/brainomics/2013_adni/MCIc-CTL-FS_cs_s_modselectcv/config_modselectcv.json --ncore 2
# 1) Log on gabriel:
ssh -t gabriel.intra.cea.fr
# 2) Run one Job to test
qsub -I
cd /neurospin/tmp/ed203246/MCIc-CTL-FS_cs_s_modselectcv
./job_Global_long.pbs
# 3) Run on cluster
qsub job_Global_long.pbs
# 4) Log out and pull Pull
exit
/neurospin/brainomics/2013_adni/MCIc-CTL-FS_cs_s_modselectcv/sync_pull.sh
# Reduce
mapreduce.py --reduce /neurospin/brainomics/2013_adni/MCIc-CTL-FS_cs_s_modselectcv/config_modselectcv.json


%run -i ~/git/scripts/2013_adni/MCIc-CTL-FS/03_tvenet_modselectcv_cs_s.py
...
WD = "/neurospin/brainomics/2013_adni/MCIc-CTL-FS_cs_s_modselectcv"
PENALTY_START = 2
def config_filenane(): return os.path.join(WD, "config_modselectcv.json")
def results_filenane(): return os.path.join(WD, "MCIc-CTL-FS_cs_s_modselectcv.xlsx")
...
def init():
    ...
    mask = np.load(STRUCTURE["mask"])
    assert X.shape[1] - PENALTY_START == mask.sum()
    from parsimony.utils.penalties import l1_max_logistic_loss
    assert l1_max_logistic_loss(X[:, PENALTY_START:], y) == 0.23959966455752704
    ...
    config = dict(data=dict(X=INPUT_DATA_X, y=INPUT_DATA_y),
                  ...
                  reduce_output="MCIc-CTL-FS_cs_s_modselectcv.csv")
...
def plot_perf():
    ...
    #INPUT_vbm = "/home/ed203246/mega/data/2015_logistic_nestv/adni/MCIc-CTL-FS/MCIc-CTL-FS_cs_s.csv"
    INPUT = os.path.join(WD, "MCIc-CTL-FS_cs_s_modselectcv.csv")
    y_col = 'recall_mean'
    x_col = 'tv'
    y_col = 'auc'
    a = 0.01
