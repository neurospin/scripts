...
adni_ref['DX.bl'] = adni_ref['DX.bl'].map({'AD': 'AD', 'LMCI': 'MCI', 'CN': 'control'})
bv_group = adni_ref[['PTID', 'DX.bl']][bl_indexes].copy() # PTID is used for index
...
adni_ref['DX'] = adni_ref['DX'].map({'Dementia': 'AD',
                        'MCI': 'MCI',
                        'NL':   'control',
                        'NL to MCI': 'MCI',
                        'NL to Dementia': 'AD',
...
Cuignet2010_group['DX'] = Cuignet2010_group['DX'].map({'AD': 'AD', 'MCI': 'MCInc', 'control': 'control'})
...
group_cmp = pandas.DataFrame.from_dict({'brainvisa_group': bv_group['DX.bl'],
                                        'Cuignet2010_group':Cuignet2010_group['DX']})
group_cmp = pandas.DataFrame.from_items([('brainvisa_group', bv_group['DX.bl']),
                                         ('Cuignet2010_group', Cuignet2010_group['DX'])])
group_cmp.sort().to_csv('groups.csv')