# Build config file
python $HOME/git/scripts/2013_adni/proj_classif_AD-CTL/02_logistictvenet.py
# 1) Log on gabriel:
...
exit
/neurospin/brainomics/2013_adni/proj_classif_AD-CTL/logistictvenet_5cv/sync_pull.sh
# Reduce
mapreduce.py --mode reduce --config /neurospin/brainomics/2013_adni/proj_classif_AD-CTL/logistictvenet_5cv/config.json

################################################################################
# Start by running Locally with 2 cores, to check that everything os OK)
Interrupt after a while CTL-C
mapreduce.py --mode map --config /neurospin/brainomics/2013_adni/proj_classif_AD-CTL/logistictvenet_5cv/config.json --ncore 2
# 1) Log on gabriel:
ssh -t gabriel.intra.cea.fr
# 2) Run one Job to test
qsub -I
...
mapreduce.py --mode map --config /neurospin/tmp/brainomics/2013_adni/proj_classif_AD-CTL/logistictvenet_5cv/config.json --ncore 2
# Run on the cluster with 30 PBS Jobs
mapreduce.py --pbs_job --config /neurospin/tmp/brainomics/2013_adni/proj_classif_AD-CTL/logistictvenet_5cv/config.json


# 1) Push your file to gabriel, run:
/neurospin/tmp/brainomics/2013_adni/proj_classif_AD-CTL/logistictvenet_5cv/sync_push.sh
# 2) Log on gabriel:
ssh -t gabriel.intra.cea.fr "cd /neurospin/tmp/brainomics/2013_adni/proj_classif_AD-CTL/logistictvenet_5cv ; bash"
# 3) Run the jobs
