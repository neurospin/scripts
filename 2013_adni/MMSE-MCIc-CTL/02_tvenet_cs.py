...
def run_all(config):
    import mapreduce
    WD = "/neurospin/brainomics/2013_adni/MMSE-MCIc-CTL"
...
if __name__ == "__main__":
    ...
    # parameters grid
    # Re-run with
    tv_range = np.array([1e-3, 5e-3, 1e-2, 5e-2, .1, .2, .3, .333, .4, .5, .6, .7, .8, .9])
    ...
    user_func_filename = os.path.join(os.environ["HOME"],
        "git", "scripts", "2013_adni", "MMSE-MCIc-CTL",
        "02_tvenet_cs.py")
    ...
    config = dict(data=dict(X=INPUT_DATA_X, y=INPUT_DATA_y),
                  ...
                  reduce_output="MMSE-MCIc-CTL.csv")
