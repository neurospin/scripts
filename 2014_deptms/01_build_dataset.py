...
MODALIES = {MRI, PET, MRI+PET}
Read the data (clinic data, ROI, MRI AND PET images)
For each pair (modality, ROI):
- Construct the masks, Build and save the datasets given a modality for:
...
- Construct the matrix X and y for the regression.
    - X is constructed for each (modality, ROI). Each row of the X contains
    and Intercept, the age, the sex and the image of the patient).
    X is centered ans scaled.
    - y gives the response to TMS for each patient
    (0 if no response / 1 if response)

Masks, X and y for the modality MRI+PET are built by concatenating MRI and
PET masks and matrices X. X associated to modality MRI+PET contains an
Intercept, the age, the sex, MRI image and PET image.
...
for MODALITY in MODALITIES:
    ...
    if np.logical_or(MODALITY == "MRI", MODALITY == "PET"):
        ...
        mask_sub_im = nib.load(os.path.join(OUTPUT_MODALITY,
                                "mask_Maskdep-sub.nii"))
        mask_sub_arr = mask_sub_im.get_data() != 0
        mask_cort_im = nib.load(os.path.join(OUTPUT_MODALITY,
                               "mask_Maskdep-cort.nii"))
        mask_cort_arr = mask_cort_im.get_data() != 0
        # implicit maskdep as the union of all Rois mask
        maskdep = (mask_sub_arr | mask_cort_arr)
        # keep only voxels inside brain
        maskdep = (maskdep & brain_arr)
        ...
        im = nib.load(os.path.join(OUTPUT_MODALITY,
                             "mask_" + ROI + ".nii"))
        assert np.all(maskdep == im.get_data())

        X_ROI = Xtot[:, maskdep.ravel()]
        X_ROI = np.hstack([Z, X_ROI])
        X_ROI -= X_ROI.mean(axis=0)
        X_ROI /= X_ROI.std(axis=0)
        X_ROI[:, 0] = 1.
        ...
        mask_ima = nib.load(os.path.join(OUTPUT_MODALITY,
                                             "mask_brain.nii"))
        ...
        mask = np.vstack((mask_arr, mask_arr))
        #####################################################################
        # Xcsi for the whole brain
        ...
        for ROI, values in dict_rois.iteritems():
            ...
            mask_ima_roi = nib.load(os.path.join(OUTPUT_DATASET,
                                                   "MRI",
                                                   "mask_" + ROI + ".nii"))
            mask_arr_roi = mask_ima_roi.get_data() != 0
            shutil.copyfile(os.path.join(OUTPUT_DATASET, "MRI",
                                         "mask_" + ROI + ".nii"),
            ...
            mask_ROI = np.vstack((mask_arr_roi, mask_arr_roi))
            ...
            fh.write('Centered and scaled data. '\
              'Shape = (%i, %i): Intercept + Age + Gender + '\
              '(%i, %i) voxels' % \
              (n, p, mask_arr_roi.sum(), mask_arr_roi.sum()))
            fh.close()