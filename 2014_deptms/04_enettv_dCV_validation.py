...
config_file:
/neurospin/brainomics/2014_deptms/results_enettv/MRI_Roiho-hippo/config_dCV_validation.json

OUTPUT_DIRECTORY:
 /neurospin/brainomics/2014_deptms/results_enettv/MRI_Roiho-hippo/results_dCV_validation/*
OUTPUT_FILE:
 /neurospin/brainomics/2014_deptms/results_enettv/MRI_Roiho-hippo/results_dCV_validation.csv
"""
...
#    for roi in rois:
    ...
    # cv=[n_fold, valid, calib]
    cv = [[i + 1,  resample[1 + (NFOLDS + 1) * i][1],
           resample[1 + (NFOLDS + 1) * i][2]] \
                        for i in xrange(NFOLDS)]
    cv.insert(0, None)  # first fold is None : y_valid = y_calib = y

    # Summary_file
    ##############
    # Read summary file to get selcted model
    # (the one which maximize a criterion)
    summary_file = os.path.join(WD, config_selection["output_summary"])
    summary = pd.read_csv(summary_file)

    criteria = ['recall_mean', 'min_recall', 'accuracy']
    model = {}
    for criterion in enumerate(criteria):
        model[criterion] = summary['param_opt_' + criterion].values.tolist()
