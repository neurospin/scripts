...
OUTPUT_CSI = os.path.join(BASE_PATH,         "mri_wb")
OUTPUT_ATLAS = os.path.join(BASE_PATH,       "mri_wb_gtvenet")
OUTPUT_CS_ATLAS = os.path.join(BASE_PATH,    "mri_wb_cs_gtvenet")
...
assert np.sum(mask_atlas != 0) == 261608
...
out_im.to_filename(os.path.join(OUTPUT_CSI, "t_stat_rep_min_norep_mri_wb.nii.gz"))
...
out_im = nibabel.Nifti1Image(arr, affine=mask_ima.get_affine())
out_im.to_filename(os.path.join(OUTPUT_CSI, "pval-quantile_rep_min_norep_mri_wb.nii.gz"))

arr = np.zeros(mask_arr.shape); arr[mask_arr] = pvals
out_im = nibabel.Nifti1Image(arr, affine=mask_ima.get_affine())
out_im.to_filename(os.path.join(OUTPUT_CSI, "pval_rep_min_norep_mri_wb.nii.gz"))

# anatomist /neurospin/brainomics/2014_deptms/mri_wb/*.nii.gz'''