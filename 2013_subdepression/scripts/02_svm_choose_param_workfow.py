...
This script produce a workflow to perform parameter selection on ANOVA+SVM.
The parameters that are selected are:
 - k: number of features selected by sklearn.feature_selection.SelectKBest
 - C: SVM regularization parameter
 - SVM penalty
This script produces a workflow to be executed by SOMA Workflow
The results should be loaded by 02_svm_choose_param.py
...
def svm_choose_param_worfkflow(h5filename, workflow_dir,
    ...
    '''Create EPAC workflow for SVM parameter selection
       The function returns X, y and the workflow just to ease interactive manipulation.'''
...
if __name__ == '__main__':
    ...
    X, y, select_wf = svm_choose_param_worfkflow(**vars(args))
