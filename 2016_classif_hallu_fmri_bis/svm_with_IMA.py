# -*- coding: utf-8 -*-
"""
Created on Fri Sep 16 10:56:06 2016
...
#out_scores_file = '/neurospin/brainomics/2016_classif_hallu_fmri_bis/results26/multivariate_analysis/without_subject19/svm_with_reprocessIMA_no_model_selection_without_subject19/scores.txt'
out_scores_file = '/neurospin/brainomics/2016_classif_hallu_fmri_bis/results26/multivariate_analysis/svm/with_IMA_no_model_selection/scores.txt'
...
T_IMA = np.load(os.path.join(BASE_PATH,'results26','multivariate_analysis','data','T_IMA.npy'))
y_state_IMA = np.load(os.path.join(BASE_PATH,'results26','multivariate_analysis','data','y_IMA.npy'))
subject_IMA = np.load(os.path.join(BASE_PATH,'results26','multivariate_analysis','data','subject_IMA.npy'))


T = np.nan_to_num(T)

T_IMA = np.nan_to_num(T_IMA)


Tdiff=np.mean(T_IMA,axis=0)-np.mean(T[y_state==0],axis=0)
...
for c in C_range:   
    ...
    for i in range(0,number_subjects):
        if i !=19:
            ...
            ytrain=np.hstack((y_state_IMA,y_state[train_bool]))
    ...
    #filename = os.path.join(BASE_PATH,"results26","multivariate_analysis","svm","svm_with_reprocessIMA_no_model_selection","coef_mean_c=%r.nii.gz")%c
    filename = os.path.join(BASE_PATH,"results26","multivariate_analysis","svm","with_IMA_no_model_selection","coef_mean_c=%r.nii.gz")%c
