...
SORTED_SUBJECT_LIST = os.path.join(PROJECT_DIR,
                                   'data', 'sorted_subject_list.npy')
...
img = ni.load(origin_img_list[0])
# Load the first image's data (!= header)
...
# Bounding box
#exact data copy (copie conforme)
mask_data = img_data.copy()
mask_data[:] = 0.0
mask_data[(zmin + 1):(zmax - 1),
          (ymin + 1):(ymax - 1),
          (xmin + 1):(xmax - 1)] = 1.0
...
template_for_size = os.path.join(PROJECT_DIR,
                                 '2013_imagen_bmi',
                                 'data',
                                 'mask',
                                 'mask.nii')
...
pn = os.path.join(PROJECT_DIR,
                  'documents',
                  '2014jan24_Plink',
                  'bbox_stats_all-cov.nii.gz')
