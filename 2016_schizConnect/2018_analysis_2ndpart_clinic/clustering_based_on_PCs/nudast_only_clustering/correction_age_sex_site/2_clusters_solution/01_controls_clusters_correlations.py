...
correction_age_sex_site/2_clusters_solution/with_controls/labels_all.npy")
...
for key in clinic.question_id.unique():
    try:
        ...
        T,p = scipy.stats.f_oneway(df[df["labels"]=="controls"][key],df[df["labels"]==0][key],\
                 df[df["labels"]==1][key])
        print(p)
...
output = "/neurospin/brainomics/2016_schizConnect/2018_analysis_2ndpart_clinic/\
results/clustering/corrected_results/correction_age_sex_site/2_clusters_solution/with_controls/nudast"
...
ax = sns.violinplot(x="labels_name", y="vocabsca", data=df,order=["controls","cluster 1","cluster 2"])
...
T, p = scipy.stats.f_oneway(df[df["labels"]=="controls"]["cvlfps"],df[df["labels"]==0]["cvlfps"],\
                     df[df["labels"]==1]["cvlfps"])
ax = sns.violinplot(x="labels_name", y="cvlfps", data=df,order=["controls","cluster 1","cluster 2"])
plt.title("ANOVA: t = %s, and  p= %s"%(T,p))
...
T, p = scipy.stats.f_oneway(df[df["labels"]=="controls"]["matrxraw"],df[df["labels"]==0]["matrxraw"],\
                     df[df["labels"]==1]["matrxraw"])
ax = sns.violinplot(x="labels_name", y="matrxraw", data=df,order=["controls","cluster 1","cluster 2"])
plt.title("ANOVA: t = %s, and  p= %s"%(T,p))
...
T, p = scipy.stats.f_oneway(df[df["labels"]=="controls"]["dstscalc"],df[df["labels"]==0]["dstscalc"],\
                     df[df["labels"]==1]["dstscalc"])
ax = sns.violinplot(x="labels_name", y="dstscalc", data=df,order=["controls","cluster 1","cluster 2"])
plt.title("ANOVA: t = %s, and  p= %s"%(T,p))
...
T, p = scipy.stats.f_oneway(df[df["labels"]=="controls"]["sstscalc"],df[df["labels"]==0]["sstscalc"],\
                     df[df["labels"]==1]["sstscalc"])
ax = sns.violinplot(x="labels_name", y="sstscalc", data=df,order=["controls","cluster 1","cluster 2"])
plt.title("ANOVA: t = %s, and  p= %s"%(T,p))
