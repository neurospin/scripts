...
results/clustering/corrected_results/correction_age_sex_site/2_clusters_solution/with_controls"
...
ax = sns.barplot(x="labels_name", y="score", hue="PC", data=df_complete,order=["controls","cluster 1","cluster 2"])
plt.legend(loc ='lower left' )
plt.savefig(os.path.join(output,"cluster_weights.png"))

plt.figure()
sns.set_style("whitegrid")
ax = sns.barplot(x="labels_name", y="age",data=df,order=["controls","cluster 1","cluster 2"])
...
T, p = scipy.stats.f_oneway(df[df["labels"]=="controls"]["age"],df[df["labels"]==0]["age"],\
                     df[df["labels"]==1]["age"])
ax = sns.violinplot(x="labels_name", y="age", data=df,order=["controls","cluster 1","cluster 2"])
