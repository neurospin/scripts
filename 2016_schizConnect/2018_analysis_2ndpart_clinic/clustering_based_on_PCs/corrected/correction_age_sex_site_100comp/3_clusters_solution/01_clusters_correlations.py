...
correction_age_sex_site_100comp/3_clusters_solution/labels_cluster.npy")
...
results/clustering/corrected_results/correction_age_sex_site_100comp/3_clusters_solution/\
...
output = "/neurospin/brainomics/2016_schizConnect/2018_analysis_2ndpart_clinic/\
results/clustering/corrected_results/correction_age_sex_site_100comp/3_clusters_solution/nudast"
...
T, p = scipy.stats.f_oneway(df[df["labels"]==0]["dstscalc"],\
                     df[df["labels"]==1]["dstscalc"],\
                     df[df["labels"]==2]["dstscalc"])
ax = sns.violinplot(x="labels_name", y="dstscalc", data=df,order=["cluster 1","cluster 2","cluster 3"])
plt.title("ANOVA: t = %s, and  p= %s"%(T,p))
...
T, p = scipy.stats.f_oneway(df[df["labels"]==0]["sstscalc"],\
                     df[df["labels"]==1]["sstscalc"],\
                     df[df["labels"]==2]["sstscalc"])
ax = sns.violinplot(x="labels_name", y="sstscalc", data=df,order=["cluster 1","cluster 2","cluster 3"])
plt.title("ANOVA: t = %s, and  p= %s"%(T,p))
...
LABELS_DICT = {0: "cluster 1", 1: "cluster 2", 2: "cluster 3"}
df["labels_name"]  = df["labels"].map(LABELS_DICT)
df["yrschool"] =  score[np.array(np.isnan(score)==False)]
T, p = scipy.stats.f_oneway(df[df["labels"]==0]["yrschool"],\
                     df[df["labels"]==1]["yrschool"],\
