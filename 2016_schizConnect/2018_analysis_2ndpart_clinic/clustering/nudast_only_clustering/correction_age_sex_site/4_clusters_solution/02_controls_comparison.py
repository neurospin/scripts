...
labels_all_scz = np.load("/neurospin/brainomics/2016_schizConnect/2018_analysis_2ndpart_clinic/results/clustering/corrected_results/correction_age_sex_site/4_clusters_solution/labels_cluster.npy")
...
output = "/neurospin/brainomics/2016_schizConnect/2018_analysis_2ndpart_clinic/results/clustering/corrected_results/correction_age_sex_site/4_clusters_solution/with_controls"
...
ax = sns.barplot(x="labels_name", y="score", hue="PC", data=df_complete,order=["controls","cluster 1","cluster 2","cluster 3","cluster 4"])
plt.legend(loc ='lower left' )
plt.savefig(os.path.join(output,"cluster_weights.png"))

plt.figure()
sns.set_style("whitegrid")
ax = sns.barplot(x="labels_name", y="age",data=df,order=["controls","cluster 1","cluster 2","cluster 3","cluster 4"])
...
T, p = scipy.stats.f_oneway(df[df["labels"]=="controls"]["age"],df[df["labels"]==0]["age"],\
                     df[df["labels"]==1]["age"],\
                     df[df["labels"]==2]["age"],df[df["labels"]==3]["age"])
ax = sns.violinplot(x="labels_name", y="age", data=df,order=["controls","cluster 1","cluster 2","cluster 3","cluster 4"])
