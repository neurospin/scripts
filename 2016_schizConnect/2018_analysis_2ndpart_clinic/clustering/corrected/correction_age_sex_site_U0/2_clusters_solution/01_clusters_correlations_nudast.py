...
results/clustering/corrected_results/correction_age_sex_site_U0/2_clusters_solution/nudast/\
...
################################################################################
################################################################################
yrschool = df_scores["yrschool"].astype(np.float).values
...
output = "/neurospin/brainomics/2016_schizConnect/2018_analysis_2ndpart_clinic/\
results/clustering/corrected_results/correction_age_sex_site_U0/2_clusters_solution/nudast/\
clusters_clinics_p_values_age_sex_corrected.csv"
for key in clinic.question_id.unique():
    try:
        neurospycho = df_scores[key].astype(np.float).values
        ...
        df["age"] = age[np.array(np.isnan(neurospycho)==False)]
        df["sex"] = sex[np.array(np.isnan(neurospycho)==False)]
        #df["yrschool"] = yrschool[np.array(np.isnan(neurospycho)==False)]
        df["labels"]=labels_cluster[np.array(np.isnan(neurospycho)==False)]
        mod = ols("labels ~ %s + age + sex + yrschool "%(key),data = df).fit()
        if (mod.pvalues[key] <0.05):
            print(mod.pvalues[key])
            print(key)
        df_stats.loc[df_stats.clinical_scores==key,"T"] = mod.tvalues[key]
        df_stats.loc[df_stats.clinical_scores==key,"p"] = mod.pvalues[key]
...
df["labels"]=labels_cluster[np.array(np.isnan(score)==False)]
df["sansSUbtot"] =  score[np.array(np.isnan(score)==False)]
df["age"] = age[np.array(np.isnan(score)==False)]
df["sex"] = sex[np.array(np.isnan(score)==False)]
mod = ols("labels ~ sansSUbtot + age + sex ",data = df).fit()
ax = sns.violinplot(x="labels", y="sansSUbtot", data=df)
plt.title("ANOVA: t = %s, and  p= %s"%(mod.tvalues["sansSUbtot"],mod.pvalues["sansSUbtot"]))
plt.savefig(os.path.join(output,"sansSUbtot_age_asex_corr.png"))
