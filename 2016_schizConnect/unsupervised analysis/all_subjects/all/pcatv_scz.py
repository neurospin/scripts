...
OUTPUT_DIR = '/neurospin/brainomics/2016_schizConnect/analysis/all_studies/VBM/all_subjects/results/pcatv_scz'
...
CONFIGS = [[5,"5_folds_all_scz", "config.json", True]]
N_COMP = 10
...
if __name__ == "__main__":
    ...
    y = np.ones(X.shape[0])
    shutil.copy(INPUT_DATA_X, os.path.join(OUTPUT_DIR,"5_folds_all_scz"))
    shutil.copy(INPUT_MASK, os.path.join(OUTPUT_DIR,"5_folds_all_scz"))
    ...
    sync_push_filename, sync_pull_filename, WD_CLUSTER = \
        clust_utils.gabriel_make_sync_data_files(os.path.join(OUTPUT_DIR,"5_folds_all_scz"))
    cmd = "mapreduce.py --map  %s/config.json" % WD_CLUSTER
    clust_utils.gabriel_make_qsub_job_files(os.path.join(OUTPUT_DIR,"5_folds_all_scz"), cmd,walltime = "250:00:00")    