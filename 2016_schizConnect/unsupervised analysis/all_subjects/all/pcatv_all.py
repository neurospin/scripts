...
OUTPUT_DIR = '/neurospin/brainomics/2016_schizConnect/analysis/all_studies/VBM/all_subjects/results/pcatv_all'
...
CONFIGS = [[5,"5_folds_all_all", "config.json", True]]
N_COMP = 10
...
if __name__ == "__main__":
    ...
    y = np.ones(X.shape[0])
    shutil.copy(INPUT_DATA_X, os.path.join(OUTPUT_DIR,"5_folds_all_all"))
    shutil.copy(INPUT_MASK, os.path.join(OUTPUT_DIR,"5_folds_all_all"))
    ...
    sync_push_filename, sync_pull_filename, WD_CLUSTER = \
        clust_utils.gabriel_make_sync_data_files(os.path.join(OUTPUT_DIR,"5_folds_all_all"))
    cmd = "mapreduce.py --map  %s/config.json" % WD_CLUSTER
    clust_utils.gabriel_make_qsub_job_files(os.path.join(OUTPUT_DIR,"5_folds_all_all"), cmd,walltime = "250:00:00")    