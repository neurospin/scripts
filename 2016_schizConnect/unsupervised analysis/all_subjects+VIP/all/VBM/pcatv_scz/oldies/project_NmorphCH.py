...
INPUT_DATA_X = '/neurospin/brainomics/2016_schizConnect/analysis/all_studies+VIP/VBM/all_subjects/data/data_by_site/NMORPH/X.npy'
INPUT_DATA_y = '/neurospin/brainomics/2016_schizConnect/analysis/all_studies+VIP/VBM/all_subjects/data/data_by_site/NMORPH/y.npy'


X_nmorph = np.load(INPUT_DATA_X)
y_nmorph = np.load(INPUT_DATA_y)

clinic = pd.read_csv(INPUT_CLINIC_FILENAME)
pop = pd.read_csv(INPUT_POPULATION)

hallu_sipsp4_scores = np.zeros(( X_nmorph.shape[0]))
i=0
for s in pop.subjectid:
    ...
    if len(curr)>0:
        ...
        if curr[curr.question_id == "sipsp4"].empty == False:
                hallu_sipsp4_scores[i] = curr[curr.question_id == "sipsp4"].question_value.astype(np.int64).values
        else:
                hallu_sipsp4_scores[i] = np.nan
    i = i +1

hallu_sipsp4_scores_scz = hallu_sipsp4_scores[y_nmorph == 1]


assert X_nmorph.shape == (80, 125961)
X_nmorph = X_nmorph[:,2:]
assert X_nmorph.shape == (80, 125959)
...
assert U_nmorph_scz.shape == (42, 10)


for i in range(10):
    plt.figure()
    x = U_nmorph_scz[:,i][np.array(np.isnan(hallu_sipsp4_scores_scz)==False)]
    y = hallu_sipsp4_scores_scz [np.array(np.isnan(hallu_sipsp4_scores_scz )==False)]
    Coef,pvalue = scipy.stats.pearsonr(x,y)
    sns.set_style("whitegrid")
    sns.set(font_scale=1.3)
    plt.plot(U_nmorph[y_nmorph==1,i],hallu_sipsp4_scores_scz ,'o')
    plt.tight_layout()
...
all_subjects/results/pcatv_scz/projection_nmorph/panss_pos"
for i in range(10):
    plt.figure()
    x = U_nmorph_scz[:,i][np.array(np.isnan(panss_pos_scz)==False)]
    y = panss_pos_scz[np.array(np.isnan(panss_pos_scz)==False)]
    Coef,pvalue = scipy.stats.pearsonr(x,y)
    sns.set_style("whitegrid")
    sns.set(font_scale=1.3)
    plt.plot(U_nmorph[y_nmorph==1,i],panss_pos_scz,'o')
    plt.tight_layout()
    plt.xlabel("Score on component %r"%i)
...
output = "/neurospin/brainomics/2016_schizConnect/analysis/all_studies+VIP/VBM/\
all_subjects/results/pcatv_scz/projection_nmorph/panss_neg"
for i in range(10):
    plt.figure()
    x = U_nmorph_scz[:,i][np.array(np.isnan(panss_neg_scz)==False)]
    y = panss_neg_scz[np.array(np.isnan(panss_neg_scz)==False)]
    Coef,pvalue = scipy.stats.pearsonr(x,y)
    sns.set_style("whitegrid")
    sns.set(font_scale=1.3)
    plt.plot(U_nmorph[y_nmorph==1,i],panss_neg_scz,'o')
    plt.tight_layout()
    plt.xlabel("Score on component %r"%i)
...
output = "/neurospin/brainomics/2016_schizConnect/analysis/all_studies+VIP/VBM/\
all_subjects/results/pcatv_scz/projection_nmorph/halluP3"
for i in range(10):
    plt.figure()
    x = U_nmorph_scz[:,i][np.array(np.isnan(panss_scores_scz[:,2])==False)]
    y = panss_scores_scz[:,2][np.array(np.isnan(panss_scores_scz[:,2])==False)]
    Coef,pvalue = scipy.stats.pearsonr(x,y)
    sns.set_style("whitegrid")
    sns.set(font_scale=1.3)
    plt.plot(U_nmorph[y_nmorph==1,i],panss_scores_scz[:,2],'o')
    plt.tight_layout()
    plt.xlabel("Score on component %r"%i)
    plt.ylabel("Hallu score P3")
    plt.title(("Coef of correlation : %s and pvalue = %r"%(np.around(Coef,decimals=3),pvalue)))
...
################################################################################
output = "/neurospin/brainomics/2016_schizConnect/analysis/all_studies+VIP/VBM/\
all_subjects/results/pcatv_scz/projection_nmorph/boxplots"
for i in range(10):
    plt.figure()
    df = pd.DataFrame()
    ...
    df["score"] = U_nmorph[:,i]
    df["dx"] = y_nmorph
    T,pvalue = scipy.stats.ttest_ind(U_nmorph[y_nmorph==0,i],U_nmorph[y_nmorph==1,i])
    sns.set_style("whitegrid")
    sns.set(font_scale=1.3)
