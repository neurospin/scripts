...
OUTPUT_DIR = '/neurospin/brainomics/2016_schizConnect/analysis/NUSDAST/VBM/results/pcatv'
...
CONFIGS = [[5,"5_folds_NUDAST", "config.json", True]]
N_COMP = 5
...
("sparse_pca", 0.0, 0.0, 0.1),("sparse_pca", 0.0, 0.0, 1.0),("sparse_pca", 0.0, 0.0, 10.0)]
...
if __name__ == "__main__":
    ...
    y = np.ones(X.shape[0])
    shutil.copy(INPUT_DATA_X, os.path.join(OUTPUT_DIR,"5_folds_NUDAST"))
    shutil.copy(INPUT_MASK, os.path.join(OUTPUT_DIR,"5_folds_NUDAST"))
    ...
    sync_push_filename, sync_pull_filename, WD_CLUSTER = \
        clust_utils.gabriel_make_sync_data_files(os.path.join(OUTPUT_DIR,"5_folds_NUDAST"))
    cmd = "mapreduce.py --map  %s/config.json" % WD_CLUSTER
    clust_utils.gabriel_make_qsub_job_files(os.path.join(OUTPUT_DIR,"5_folds_NUDAST"), cmd,walltime = "250:00:00")    
