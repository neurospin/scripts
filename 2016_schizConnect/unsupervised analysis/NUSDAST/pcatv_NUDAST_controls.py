...
OUTPUT_DIR = '/neurospin/brainomics/2016_schizConnect/analysis/NUSDAST/VBM/results/pcatv_controls'
...
CONFIGS = [[5,"5_folds_NUDAST_controls", "config.json", True]]
N_COMP = 10
...
#    resample_index = [[tr.tolist(), te.tolist()] for tr, te in skf]
    ...
    config = dict(data=dict(X="X_controls_only.npy"),
                   structure="mask.nii",
                  params=PARAMS,
...
if __name__ == "__main__":
    ...
    y = np.ones(X.shape[0])
    shutil.copy(INPUT_DATA_X, os.path.join(OUTPUT_DIR,"5_folds_NUDAST_controls"))
    shutil.copy(INPUT_MASK, os.path.join(OUTPUT_DIR,"5_folds_NUDAST_controls"))
    ...
    sync_push_filename, sync_pull_filename, WD_CLUSTER = \
        clust_utils.gabriel_make_sync_data_files(os.path.join(OUTPUT_DIR,"5_folds_NUDAST_controls"))
    cmd = "mapreduce.py --map  %s/config.json" % WD_CLUSTER
    clust_utils.gabriel_make_qsub_job_files(os.path.join(OUTPUT_DIR,"5_folds_NUDAST_controls"), cmd,walltime = "250:00:00")    