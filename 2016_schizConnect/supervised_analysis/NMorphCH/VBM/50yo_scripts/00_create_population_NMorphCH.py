...
INPUT_DATA = "/neurospin/abide/schizConnect/data_nifti_format/NMorphCH"
...
OUTPUT_CSV = os.path.join(BASE_PATH,"population_50yo.csv")
...
subjects = list()
date = list()
paths = glob.glob(os.path.join(INPUT_DATA,"NU/*/*/mwc1average_T1.nii"))
...
assert input_subjects_vbm.shape == (127, 3)                        
...
assert  input_subjects_vbm.shape == (127, 7)
pop = input_subjects_vbm
assert  pop.shape == (127, 7)
...
assert sum(pop.dx_num.values==0) == 38
assert sum(pop.dx_num.values==1) == 42
assert sum(pop.sex_num.values==0) == 46
assert sum(pop.sex_num.values==1) == 34
