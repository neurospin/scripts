...
assert np.sum(mask_atlas != 0) ==  47151
out_im = nibabel.Nifti1Image(mask_atlas,
                             affine=babel_image.get_affine())
out_im.to_filename(os.path.join(OUTPUT, "mask_res_3mm.nii"))
im = nibabel.load(os.path.join(OUTPUT, "mask_res_3mm.nii"))
...
out_im = nibabel.Nifti1Image(mask_bool.astype("int16"),
                             affine=babel_image.get_affine())
out_im.to_filename(os.path.join(OUTPUT, "mask_res_3mm.nii"))
babel_mask = nibabel.load(os.path.join(OUTPUT, "mask_res_3mm.nii"))
...
X[:, 0] = 1.
n, p = X.shape
np.save(os.path.join(OUTPUT, "X_res_3mm.npy"), X)
np.save(os.path.join(OUTPUT, "y_res_3mm.npy"), y)
