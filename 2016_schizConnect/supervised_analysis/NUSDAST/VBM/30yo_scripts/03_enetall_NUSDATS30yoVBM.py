...
WD = '/neurospin/brainomics/2016_schizConnect/analysis/NUSDAST/VBM/results_30yo/enetall_NUSDATS30yoVBM'

WD_CLUSTER = WD.replace("/neurospin/", "/mnt/neurospin/sel-poivre/")
DATA_PATH = '/neurospin/brainomics/2016_schizConnect/analysis/NUSDAST/VBM/data/data_30yo'
WD_ORIGINAL = '/neurospin/brainomics/2016_schizConnect/analysis/NUSDAST/VBM/results_30yo'
user_func_filename = '/home/ed203246/git/scripts/2016_schizConnect/supervised_analysis/NUSDAST/VBM/30yo_scripts/03_enetall_NUSDATS30yoVBM.py'
...
def init():
    ...
    cv_old = json.load(open(os.path.join(WD_ORIGINAL, "enettv", "enettv_NUDAST_30yoVBM", "config_dCV.json")))["resample"]
...
def load_globals(config):
    ...
    assert np.allclose(Agn.get_singular_values(0), 11.909770107366217, rtol=1e-03, atol=1e-03)
...
def do_sync():
    ...
    # copy enetgn
    SRC = os.path.join(WD_ORIGINAL, "enetgn", "enetgn_NUDAST_30yo", "model_selectionCV/%s/%s")
    DST = os.path.join(WD, "5cv/%s/%s")
    sync(SRC, DST, outer_str, inner_str, dst_prefix="enetgn")

    SRC = os.path.join(WD_ORIGINAL, "enettv", "enettv_NUDAST_30yoVBM", "model_selectionCV/%s/%s")
    DST = os.path.join(WD, "5cv/%s/%s")
