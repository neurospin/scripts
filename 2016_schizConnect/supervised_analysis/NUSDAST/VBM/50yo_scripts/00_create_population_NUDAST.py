...
INPUT_DATA = "/neurospin/abide/schizConnect/data_nifti_format/NUSDAST"
...
assert  input_subjects_vbm.shape == (391, 7)
pop = input_subjects_vbm
assert  pop.shape == (391, 7)
...
assert sum(pop.dx_num.values==0) == 139
assert sum(pop.dx_num.values==1) == 97
assert sum(pop.sex_num.values==0) == 135
assert sum(pop.sex_num.values==1) == 101
