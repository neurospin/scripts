...
OUTPUT_CSV = os.path.join(BASE_PATH,"Freesurfer","population_without_sibling.csv")


# Read clinic data
clinic = pd.read_csv(INPUT_CLINIC_FILENAME)
clinic = clinic[clinic.study=="NUSDAST"]
# Read free surfer assembled_data
input_subjects_fs = dict()
paths = glob.glob(INPUT_FS+"/*_lh.mgh")
for path in paths:
    subject = os.path.basename(path)[19:-7]
    date = os.path.basename(path)[8:-14]
    id = os.path.basename(path)[8:-7]
...
assert input_subjects_fs.shape == (387, 5)
...
assert  input_subjects_fs.shape == (387, 9)


pop = input_subjects_fs
assert  pop.shape == (387, 9)


# Map group
DX_MAP = {'No_Known_Disorder': 0, 'Sibling_of_No_Known_Disorder':0 ,'Schizophrenia_Strict': 1}
pop['dx_num'] = pop["dx"].map(DX_MAP)
assert sum(pop.dx_num.values==0) == 222
assert sum(pop.dx_num.values==1) == 165

SEX_MAP = {'male': 0, 'female': 1}
pop['sex_num'] = pop["sex"].map(SEX_MAP)
assert sum(pop.sex_num.values==0) ==213
assert sum(pop.sex_num.values==1) == 174
...
assert  pop.shape == (269, 11)
 


  # remove sibling of controls
pop = pop[pop["dx"]!="Sibling_of_No_Known_Disorder"]
assert  pop.shape == (219, 11)
