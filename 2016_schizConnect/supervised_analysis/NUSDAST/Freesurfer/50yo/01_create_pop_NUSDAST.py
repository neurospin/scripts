...
OUTPUT_CSV = os.path.join(BASE_PATH,"Freesurfer","population_50yo.csv")
...
assert input_subjects_fs.shape == (371, 5)
...
assert  input_subjects_fs.shape == (371, 9)


pop = input_subjects_fs
assert  pop.shape == (371, 9)
...
pop = pop[pop.age<50]
assert sum(pop.dx_num.values==0) == 131
assert sum(pop.dx_num.values==1) == 95
assert sum(pop.sex_num.values==0) == 129
assert sum(pop.sex_num.values==1) == 97
