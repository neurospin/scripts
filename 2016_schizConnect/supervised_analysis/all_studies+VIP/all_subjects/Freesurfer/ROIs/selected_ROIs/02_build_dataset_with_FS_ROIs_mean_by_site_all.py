...
####################################################################################
#REMOVE ROIs IMAPCTED BY ANTIPSUCHOTIC TREATMENT
table = pop.merge(vol_all, on="id")
y = np.asarray(table["dx_num"])
cov = table[["age",'sex_num']]
cov = np.asarray(cov)
...
ROIs =  (["age",'sex_num',
       'Left-Cerebellum-White-Matter', 'Left-Cerebellum-Cortex',
       'Left-Thalamus-Proper','Brain-Stem',
       'Left-Hippocampus', 'Left-Amygdala', 'CSF', 'Left-Accumbens-area',
       'Left-VentralDC', 'Left-choroid-plexus',
       'Right-Cerebellum-White-Matter', 'Right-Cerebellum-Cortex',
...
table = table[ROIs]
assert table.shape == (567, 43)
features = table.keys()
stats = np.asarray(table)
Xv = stats.astype(float)
assert Xv.shape == (567,43)
...
np.save(os.path.join(OUTPUT_DATA,"Xrois_volumes_mean_centered_by_site+cov_no_striatum.npy"),Xv)
np.save(os.path.join(OUTPUT_DATA,"y.npy"),y)
np.save(os.path.join(OUTPUT_DATA,"features_no_striatum.npy"),features)
