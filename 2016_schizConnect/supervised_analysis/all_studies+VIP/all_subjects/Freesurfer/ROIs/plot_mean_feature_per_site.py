...
#Before correction
###############################################################################
INPUT_DATA_X = "/neurospin/brainomics/2016_schizConnect/analysis/all_studies+VIP/Freesurfer/all_subjects/data/data_ROIs/Xrois_volumes.npy"
...
plt.xlabel("sites")
plt.ylabel("Mean Volume across ROIs")



df = pd.DataFrame()
df["Sites"] =site
df["Mean Volume across ROIs"] = X.mean(axis=1)
df["y"] = y
...
ax = sns.violinplot(x="Sites", y="Mean Volume across ROIs", hue="Subjects", data=df,\
                    split=True,linewidth = 3)
plt.tight_layout()
plt.legend(loc='lower center',ncol=2)
# statistical annotation
x1, x2 = 0, 3
y, h, col = df['Mean Volume across ROIs'].max() + 25000, 11000, 'k'
plt.plot([x1, x1, x2, x2], [y, y+h, y+h, y], lw=1.5, c=col)
plt.text((x1+x2)*.5, y+h+1000, "F = 1.45, p = 0.22", ha='center', va='bottom', color=col,size = 14)

plt.savefig("/neurospin/brainomics/2016_schizConnect/analysis/all_studies+VIP/Freesurfer/all_subjects/results/ROIs_analysis/boxplot_ROIs_features.png")
