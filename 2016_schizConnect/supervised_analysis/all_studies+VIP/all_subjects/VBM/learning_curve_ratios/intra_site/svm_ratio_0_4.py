...
WD = '/neurospin/brainomics/2016_schizConnect/analysis/all_studies+VIP/VBM/all_subjects/results/learning_curve_ratios/inter_site/ratio_0.4'
...
user_func_filename = '/home/ad247405/git/scripts/2016_schizConnect/supervised_analysis/all_studies+VIP/all_subjects/VBM/learning_curve_ratios/intra_site/svm_ratio_0_4.py'
...
def init():
    ...
    cv_outer[0][0] = cv_outer[0][0][:int(np.around(len(cv_outer[0][0])*0.4))]
    cv_outer[1][0] = cv_outer[1][0][:int(np.around(len(cv_outer[1][0])*0.4))]
    cv_outer[2][0] = cv_outer[2][0][:int(np.around(len(cv_outer[2][0])*0.4))]
    cv_outer[3][0] = cv_outer[3][0][:int(np.around(len(cv_outer[3][0])*0.4))]
    cv_outer[4][0] = cv_outer[4][0][:int(np.around(len(cv_outer[4][0])*0.4))]
