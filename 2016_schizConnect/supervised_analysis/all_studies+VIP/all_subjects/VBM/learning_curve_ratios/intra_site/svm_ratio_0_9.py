...
WD = '/neurospin/brainomics/2016_schizConnect/analysis/all_studies+VIP/VBM/all_subjects/results/learning_curve_ratios/intra_site/ratio_0.9'
WD_CLUSTER = WD.replace("/neurospin/", "/mnt/neurospin/sel-poivre/")

DATA_PATH = '/neurospin/brainomics/2016_schizConnect/analysis/all_studies+VIP/VBM/all_subjects/data/mean_centered_by_site_all'
user_func_filename = '/home/ad247405/git/scripts/2016_schizConnect/supervised_analysis/all_studies+VIP/all_subjects/VBM/learning_curve_ratios/intra_site/svm_ratio_0_9.py'
...
NFOLDS_OUTER = 5 #number of sites
NFOLDS_INNER = 5
penalty_start = 3
#DATA_TYPE = "mesh"
DATA_TYPE = "image"
...
def init():
    ...
    assert len(cv_outer[0][0]) == 484
    assert len(cv_outer[1][0]) == 485
    assert len(cv_outer[2][0]) == 485
    assert len(cv_outer[3][0]) == 485
    assert len(cv_outer[4][0]) == 485



    cv_outer[0][0] = cv_outer[0][0][:int(np.around(len(cv_outer[0][0])*0.9))]
    cv_outer[1][0] = cv_outer[1][0][:int(np.around(len(cv_outer[1][0])*0.9))]
    cv_outer[2][0] = cv_outer[2][0][:int(np.around(len(cv_outer[2][0])*0.9))]
    cv_outer[3][0] = cv_outer[3][0][:int(np.around(len(cv_outer[3][0])*0.9))]
    cv_outer[4][0] = cv_outer[4][0][:int(np.around(len(cv_outer[4][0])*0.9))]
