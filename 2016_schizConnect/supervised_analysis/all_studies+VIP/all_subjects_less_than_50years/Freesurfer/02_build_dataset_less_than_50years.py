...
BASE_PATH = "/neurospin/brainomics/2016_schizConnect/analysis/all_studies+VIP/Freesurfer/all_subjects_less_than_50years"
...
np.save('/neurospin/brainomics/2016_schizConnect/analysis/all_studies+VIP/Freesurfer/all_subjects_less_than_50years/data/site.npy',pop["site_num"].as_matrix())
#############################################################################
## Build mesh template
...
assert Xtot.shape == (490, 327684)

mask = ((np.max(Xtot, axis=0) > 0) & (np.std(Xtot, axis=0) > 1e-2))
assert mask.sum() ==  299853

np.save(os.path.join(OUTPUT, "mask.npy"), mask)

X = Xtot[:, mask]
assert X.shape == (490, 299853)


#############################################################################

X = np.hstack([Z, X])
assert X.shape == (490, 299856)
#Remove nan lines
...
assert np.all([a.shape == (299853, 299853) for a in Atv])
