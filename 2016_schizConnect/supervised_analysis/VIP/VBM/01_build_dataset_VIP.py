...
n = len(pop)
assert n == 92
Z = np.zeros((n, 2)) # Intercept + Age + Gender
Z[:, 0] = 1 # Intercept
y = np.zeros((n, 1)) # DX
images = list()
for i, index in enumerate(pop.index):
    ...
    Z[i, 1:] = np.asarray(cur[["sex_code"]]).ravel()
    y[i, 0] = cur["dx"]
...
assert np.sum(mask_atlas != 0) ==   250022
