...
INPUT_FS = "/neurospin/brainomics/2016_schizConnect/analysis/COBRE/Freesurfer/data/freesurfer_assembled_data_fsaverage"
...
for path in paths:
    subject = os.path.basename(path)[17:-7]# 17 first car are COBRE+data, 7 last are extension
    date = os.path.basename(path)[6:-17]
    id = os.path.basename(path)[6:-7]
...
assert input_subjects_fs.shape == (156, 5)
...
assert  input_subjects_fs.shape == (156,9)


pop = input_subjects_fs
assert  pop.shape == (156, 9)
...
assert sum(pop.dx_num.values==0) == 85
assert sum(pop.dx_num.values==1) == 69


assert sum(pop.sex_num.values==0) == 115
assert sum(pop.sex_num.values==1) == 39
