...
WD = '/neurospin/brainomics/2016_schizConnect/analysis/COBRE/VBM/results/enettv/enettv_model_selection_5folds_COBRE'
...
#    paths = [p for p in paths if not p.count("0.01_")]
    ...
    print('## -----------------------------------------')
    data = list()
    bycv = groupby_paths([p for p in paths if p.count("cvnested") and not p.count("all/cvnested")  ], 1)
    ...
    print('## ---------------------------')
    scores_l1l2tv = scores("nestedcv", [os.path.join(config['map_output'], row["fold"], "all", row["param_key"]) for index, row in l1l2tv.iterrows()], config)
...
if __name__ == "__main__":
    WD = '/neurospin/brainomics/2016_schizConnect/analysis/COBRE/VBM/results/enettv/enettv_model_selection_5folds_COBRE'    
    INPUT_DATA_X = '/neurospin/brainomics/2016_schizConnect/analysis/COBRE/VBM/data/X.npy'
    INPUT_DATA_y = '/neurospin/brainomics/2016_schizConnect/analysis/COBRE/VBM/data/y.npy'
    INPUT_MASK_PATH = '/neurospin/brainomics/2016_schizConnect/analysis/COBRE/VBM/data/mask.nii'
    INPUT_CSV = '/neurospin/brainomics/2016_schizConnect/analysis/COBRE/VBM/population.csv'
...
#    alphas = [.1]
    ...
    user_func_filename = "/home/ad247405/git/scripts/2016_schizConnect/supervised_analysis/COBRE/VBM/02_enettv_COBRE.py"
