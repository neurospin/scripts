...
BASE_PATH = "/neurospin/brainomics/2016_schizConnect/analysis/all_studies/Freesurfer/all_subjects_less_than_30years"
...
assert sum(pop.site_num.values==1) == 58
assert sum(pop.site_num.values==2) == 33
assert sum(pop.site_num.values==3) == 170



assert sum(pop.dx_num.values==0) == 157
assert sum(pop.dx_num.values==1) == 104

assert sum(pop.sex_num.values==0) == 158
assert sum(pop.sex_num.values==1) == 103
